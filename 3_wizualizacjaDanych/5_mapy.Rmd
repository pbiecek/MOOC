...
date: "sezon 2 / odcinek 5 <br><br><br><br>Naciśnij `A` by zamienić slajdy na ciągły tekst. <br> Naciśnij `T` by wyświetlić spis treści."
output:
  slidy_presentation:
    includes:
...
Aby przedstawiać informacje na mapach w programie R, należy wpierw wczytać obrysy obszarów z map.
...
Pliki z kształtami województw Polsi, które wykorzystam w tym odcinku, pochodzą z bazy danych GADM http://www.gadm.org i mają pewne niewielkie błędy w kodowaniu, które ręcznie naprawimy. 
...
```{r, fig.height=4, fig.width=5, message=FALSE, warning=FALSE}
# zanim zaczniemy pracować, potrzebujemy wczytać kilka bibliotek
library(maptools)
library(rgdal)
library(gpclib)
gpclibPermit()
# wczytujemy dane o kształtach województw
...
Przykłądowo, zmienna `shp1@data` zawiera zbiór danych dołączony do kształtów, a w tym konkretnym pliku `shp1@data$VARNAME_1` zawiera nazwy województw.
...
Ale aby zaznaczyć wartości liczbowe na mapie za pomocą koloru wypełnienia, do tego użyjemy geometrii `geom_map()`. 
...
Poniższy przykład rysuje kartogram, w którym infomacja o procencie oddanych ważnych głosów przedstawiamy kolorem wypełnienia
...
Mapy można rysować w różnych projekcjach, nie ma jednej idealnej. Niektóre zachwouja lepiej pola, inne kąty, inne długości odcinków.Listę projekcji z opisem właściwości można znaleźć pod adresem http://en.wikipedia.org/wiki/List_of_map_projections
...
# przygotowujemy wykres, tak jak w poprzednim odcinku
mapka <- ggplot() +
    geom_map(data=mandatySejmik2014, aes(map_id=Wojewodztwo, fill=ProcentWaznychGlosow), 
             map=shp1f) +
    geom_path(data=shp1f, aes(x=long, y=lat, group=id), colour="black", size=0.25) 

# przekształcamy mapę, stosując projekcję mercator (odwzorowanie walcowe równokątne)
...
Narysowaliśmy już kilka kartogramów. Ale nei są one jeszcez najpiękniejsze. Jak je modyfikować?

Mapy wykonane z pakietem `ggplot2` modyfikuje się podobnie jak inne wykresy w pakiecie `ggplot2`. 

Na poniższym przykłądzie przedstawiamy kilka przykładowych zmian. Funkcją `theme_bw()` zmieniamy domyślny motyw wykresu, funkcją `scale_fill_gradient()` zmieniamy sposób reprezentacji zmiennej za pomocą koloru wypełniania, a dodatkowymi argumentami funkcji `theme()` usuwamy wszystkie dodatkowe elementy wykresu.
...
ggplot() +
  ...
  theme_bw() +
  # zmienną ilosciową będizemy prezentować na skali kolorów od szarości po czerń
...
Nie zawsze przedstawianie danych za pomocą koloru wypełneinia to dobry pomysł.
...
ggplot() +
  ...
  # zmieniamy motyw graficzny, usuwając szare tło
  theme_bw() +
  # zmienną ilosciową będziemy prezentować na skali wielkości punktów od 1 do 20
...
Na poniższym wykresie realizujemy ten pomysł. Dodajemy słupki prezentujące liczbę zdobytych mandatów przez PiS, PSL i SLD, każdą partię oznaczamy innym kolorem. Łatwiej dzięki temu zauważyć, któ®a partia wygrała w którym województwie i z jaką przewagą.

```{r, fig.height=4, fig.width=5, message=FALSE, warning=FALSE}
ggplot() +
  geom_path(data=shp1f, aes(x=long, y=lat, group=id), colour="grey", size=0.25) +
  # warstwa z paskami, krezentująca liczbę mandatów zdobytych przez PO
  geom_rect(data=mandatySejmik2014, aes(xmin=long-0.3, xmax=long-0.1, 
                                        ymin=lat, ymax=lat + PO/15), fill="orange3", color="white") +
  # warstwa z paskami, krezentująca liczbę mandatów zdobytych przez PiS
  geom_rect(data=mandatySejmik2014, aes(xmin=long-0.1, xmax=long+0.1, 
                                        ymin=lat, ymax=lat + PiS/15), fill="blue3", color="white") +
  # warstwa z paskami, krezentująca liczbę mandatów zdobytych przez PSL
  geom_rect(data=mandatySejmik2014, aes(xmin=long+0.1, xmax=long+0.3, 
                                        ymin=lat, ymax=lat + PSL/15), fill="green3", color="white") +
  # warstwa z paskami, krezentująca liczbę mandatów zdobytych przez SLD
