...
- Jak dodać annotacje do wykresu?
...
Będziemy tworzyć bardzo dobre wykresy, poniewaz na naszych wykresach wszystko będzie jasne i opisane.
Przyjrzyjmy się wykresowi z pierwszego odcinka i zastnaówmy się któ®e elementy warto dodatkowo opisać?
Poświęć kilka minut by znaleźć przynajmniej pięć elmentów, które warto lepiej opisać na wym wykresie.
...
Mogło tak być, ponieważ widzieliśmy ten wykres juz wiele razy i wiemy co on przedstawia. Jeżeli jednak udało Ci się znaleźć elementy, warto dodakotwego opsiu to świetnie, jesteś na dobrej ścieżce.
...
- Opis legendy 'druzyna' możę i jest zabawny ale niewiele mówi. Jaka drużyna? Dla kogoś, kto nie wie o wyścigach gatunków ten napis nic nie powie.
...
Zobaczmy jak przygotować wykres z odpowiednimi anotacjami

---

# Annotacje

W poniższym kodzie do wykresu dodajemy cztery linijki, każda zmieniająca okreśłony aspekt wykresu. Funkcja `ggtitle()` dodaje tytuł do wykresu a funkcją `theme()` zmieniamy wielkość tytułu wykresu, większy będzie wyraźniejszy. Funkcjami `xlab()` i `ylab()` dodajemy opsiy dla osi. Aaby zmienić tytuł legendy należy określić argument `name` (nazwa) dla funkcji opisującej mapowanie na rozmiar (`scale_shape_discrete()`), a funkcją `theme()` przesuwamy tę legendę na górę wykresu. Funkcją `geom_text()` dodajemy etykiety dla wybranych punktów na wykresie.
...
Zobaczmy jak wygląda kontrolowanie mapowaniami zmiennych ciągłych. Tym razem na kolor i wielkość punktu zamapujemy ciągłą zmienną `zywotnosc`. Jeżeli nie podoba nam się domyślny dobór kolorów, to funkcją `scale_color_gradient()` możemy określić pomiędzy jakimi wartościami `low=` i `high=` ma rozpinać się skala kolorów. Dla wielkości puntku określamy jaki przedział żywotności ma być prezentowany `limits=` oraz na jakiej wielkości punkty ten przedział ma być przedstawiony `range=`.
...
Do tego miejsca mogło nam trochę obrzygnąć szare tło na wykresach. Okazuje się, że w prosty sposób mozna zmienić wygląd wykresu, używając motywów, czyli kompletów ustawień graficznych. Wiele takich motywów dostępnych jest w pakiecie `ggthemes`. Aby zmienić wygląd wykresu wystarczy dodać funkcję z motywem. Nazwy tych funkcji rozpoczynają się od `theme_`. Poniżej przykłąd dla czterech motywów graficznych.
