...
values=c("K", "P"), breaks=c("Ptak", "Kot")
kod sugeruje że K odnosi się do "Ptak"
"blue3" odnosi się do "Ptak"
...
ustalić taką samą kolejność liczb w legendzie:
np. od 10 do 50 lub od 50 do 10
...
wyjaśnić co oznaczają liczby w legend.position
...
warto pokazać opcję: theme_bw()+theme(plot.background = element_rect(fill = "gray80"))

na koniec pokazać przykłady - nawek jako "ciekawostki":
pakiet - `tikzDevice` - wykresy jak w Latex-u
pakiet `xkcd` oraz `extrafont`
http://davetang.org/muse/2013/05/21/creating-plots-using-the-xkcd-package-in-r/

powiedzieć o różnych możliwościach rozmieszczeniach wykresów
grid::pushViewport
