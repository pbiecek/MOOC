...
Wszystko to możemy zrobić, najczęściej wystarczy zmodyfikować poszczególne argumenty motywu, używając funkcji `theme()`.
...
Jednym z argumentów tej funkcji `print()` dla wykresów, jest argument `vp=` pozwalający określić w którym miejscu wykres ma być rysowany. 
...
Poniższy przykład rysuje ten sam wykres trzykrotnie, za każdym razem w innym miejscu i z innym tytułem. Argumenty `x` i `y` określają środek zarysowanego obszaru, a `width` i `height` szerokość i wysokość zarysowanego prostokątu.

```{r, fig.height=4, fig.width=5, message=FALSE, warning=FALSE}
library(grid)
# rysujemy wykres w kwadracie o szerokości i wysokości 0.5 ekranu, w lewym górnym rogu 
print(pl_waga + ggtitle("Wykres 1"), 
      vp=viewport(x=0.25, y = 0.75, width=0.5, height=0.5))
# rysujemy wykres w kwadracie o szerokości i wysokości 0.5 ekranu, w lewym dolnym rogu 
print(pl_waga + ggtitle("Wykres 2"), 
      vp=viewport(x=0.25, y = 0.25, width=0.5, height=0.5))
# rysujemy wykres w prostokącie o szerokości 0.5 a wysokości pełnego ekranu, po prawej stronie 
...
Argument `vp=` pozwala na rysowanie w dolonym elemencie wykresu. 

Ale jeżeli chcemy rysować wykresy na regularnej siatce to wygodnie jest najpierw określić tę siatkę funkcją `grid.layout()` a następnie wypełnić ją wykresami wskazując pozycje siatki do zarysowania.

```{r, fig.height=4, fig.width=5, message=FALSE, warning=FALSE}
# siatka o rozmiarze dwa wiersze i dwie kolumny
pushViewport(viewport(layout = grid.layout(2,2)))
# wklejanie wykresow do wybranych komórek
...
Okazuje się, że w prosty sposób można zmienić wygląd całego wykresu, używając tzw. motywów, czyli kompletów ustawień graficznych. Wiele takich motywów dostępnych jest w pakiecie `ggthemes`. Aby zmienić wygląd wykresu wystarczy dodać funkcję z motywem. Nazwy tych funkcji rozpoczynają się od `theme_`. 

Poniżej przykład dla czterech motywów graficznych, warto zobaczyć jak wyglądają pozostałe.

```{r, fig.height=4, fig.width=5, eval=FALSE}
# przygotowujemy wykres puntkowy
pl <- ggplot(koty_ptaki, aes(x=waga, y=predkosc, shape=druzyna)) +
  geom_point(size=5) 
# pakiet z dodatkowymi motywami 
# jeżeli nie jest zainstalowany, należy go dodać funkcją install.packages()
library(ggthemes)
