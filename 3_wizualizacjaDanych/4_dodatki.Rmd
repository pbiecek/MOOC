...
Wystarczy jako kolejne argumenty podać wykresy, które mają być wyświetlone. Funkcja `grid.arrange()` sama je uporządkuje. Argumentem `ncol` można określić w ilu kolumnach mają być wyświetlane wykresy.

*Uwaga!* Funkcja `grid.arrange()` jest dostępna w pakiecie `gridExtra`. Ten pakiet nie jest dostępy w podstawowej dystrybucji R. Należy go doinstalować funkcją `install.packages()`.
