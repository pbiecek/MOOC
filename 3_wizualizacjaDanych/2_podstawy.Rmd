...
Funkcją `library()` włączamy pakiet i możemy sprawdzić jak wyglądają pierwsze wiersze z obu zbiorów danych.
...
# jeżeli ten pakiet nie jest zainstalowany, należy go zainstalować poleceniem install.packages("ggplot2")
library(ggplot2)
...
Na wykresie punktowym można również zaznaczyć grup wierszy. Oznaczając je kolorem, kształtem lub wielkością. Pozwala to dodatkowo na porównanie grup pod względem przedstawianych zmiennych (patrz slajdy 13-16). Taki wykres punktowy z grupami warto stosować gdy:
...
Spróbujmy tego drugiego rozwiązania. Czytają plik z pomocą dla geometrii `geom_text()` można zauważyć, że za pozycjonowanie odpowiadają cechy `hjust` (w poziomie) i `vjust` (w pionie). Zobaczmy jak wyglądać będzie użycie tych argumentów.
...
2. Jeżeli etykiety są zbyt i przestają być czytelne, to można je skracać do kilku pierwszych liter np. funkcją `substr()`.
...
Jest to o tyle ciekawy przykład, że wymagać będzie wskazania innej cechy niż `y`, tak się bowiem składa, że wstęgi są opisane przedziałem, czyli cechami `ymin` i `ymax`. Każdą z tych cech możemy połączyć ze zmienną lub przypisać do nich stałą wartość. 
