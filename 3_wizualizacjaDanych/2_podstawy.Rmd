...
Dane są czymś abstrakcyjnym i patrząc w tabele liczb często trudno zrozumieć zależności pomiędzy różnymi zmiennymi.

Ale znacznie łatwiej dostrzec lub zrozumieć te zależności, jeżeli dane przedstawi się graficznie w poprawny sposób.

Pierwsza połowa seoznu 2 poświęcona jest wizualizacji danych. W tym odcinku poznamy pakiet graficzny `ggplot2`, pozwalający na tworzenie dobrze wyglądających i czytelnych wykresów.
...
W tym odcinku będziemy pracować z dwoma zbiorami danych: `koty_ptaki` oraz `WIG`. Oba zbiory są dostępne z pakietu `PogromcyDanych`.
...
Funkcją `library()` włączamy pakiet i mozęmy sprawdzić jak wyglądają pierwsze wiersze z obu zbiorów danych.
...
# wyświtlamy pierwsze wiersze z interesującego nas zbioru danych
head(koty_ptaki)
...
- Pakiet `ggplot2` pozwala na tworzenie grafiki *publication ready*, czyli wystarczająco dobrej by pokazać ją innym osobom bez dodatkowych zabiegów upiększających. 
...
# bez ładowania dodatkowych pakietów, możemy korzystać z funkcji plot()
plot(koty_ptaki$waga, koty_ptaki$predkosc)

# funkcje graficzne, które będziemy omawiać, wymagają włączenia dodatkowego pakietu ggplot2
# jeżęli ten pakiet nie jest zainstalowany, należy go zainstalować poleceniem install.packages("ggplot2")
library(ggplot2)
# w pakiecie ggplot2 wykresy wygodnie tworzy się funkcją ggplot
...
Dla kolejnych wierszy zbioru danych `koty_ptaki`, na wykresie przedstawiamy zależność pomiędzy zmiennymi `waga` a `prędkosc`. Każdy wiersz opisany jest przez jeden punkt, przez co wykres ten nazywa się wykresem punktowym.

Na tym wykresie przedstawiane są duże koty i ptaki. Lżejsze gatunki to głównie ptaki, które są też szybsze niż duże koty. Więc przedstawiany wykres pokazuje, że im cięższy osobnik, tym niższa jego prędkość maksymalna.
...
Na wykresie punktowym można również zaznaczyć grup wierszy. Oznaczając je kolorem, kształtem lub wielkością. Pozwala to dodatkowo na porównanie grup pod względem przedstawianych zmiennych (patrz slady 13-16). Taki wykres punktowy z grupami warto stosować gdy:

1. Nie wiemy czy grupy różnią się pod kątem danej pary zmiennych i chcemy to sprawdzić (np. czy auta 3 drzwiowe różnią się od 5 drzwiowych jeżeli chodzi o cenę i przebieg).
...
# pierwsze trzy wiersze ze zbioru danych
head(koty_ptaki, 3)
# instrukcja rysująca wykres, poniżej wynik
ggplot(koty_ptaki, aes(x=waga, y=predkosc)) +
  geom_point()
```
...
- Funkcja `ggplot()` tworzy trzon wykresu. Opisuje ona jakie zmienne będą przedstawione na wykresie, ale nie określa w jaki sposób. Zazwyczaj przyjmuje ona dwa argumenty. Pierwszy to zbiór danych (w tym przypadku `koty_ptaki`). Drugi argument to lista par, określająca które zmienne i w jaki sposób mają być przedstawione na wykresie. Te pary nazywa się  ,,mapowaniami'', określają one które zmienne mają być przedstawione za pomocą jakich cech wykresu. 
...
Jak dotąd, nie widać jeszcze jak potężnym mechanizmem są mapowania. Zademonstrujmy to w kolejnym przykładzie, gdzie na wykresie zaznaczymy grupy punktów, gdzie grupy określa zmienna `druzyna`. Grupy punktów oznaczymy różnymi kształtami, a więc cechą `shape`.
...
# pierwsze trzy wiersze ze zbioru danych
head(koty_ptaki, 3)
# wykres punktowy z grupami zaznaczonymi różnymi kształtami punktów
...
Spójrzmy raz jeszcze na polecenie twrzące wykres i przeanalizujemy jak wygląda sposób myślenia przy tworzeniu wykresu.
...
Jeżeli efekt oznaczania grup nam się nie podoba, możemy wykorzystać inną cechę do rozróżnienia kotów od ptaków. 
...
Jeżeli wynik jest wciąż niezadawalający, możemy próbować zmieniać kolejne cechy.
...
Czyli kolor wykresu odpowiadać może tylko jednej kolumnie w zbiorze z danymi, kształt punktu też tylko jednej kolumnie w zbiorze z danymi i podobnie inne cechy. Ale jedna kolumna w zbiorze z danymi może być przedstawiana i przez kolor i przez kształt.

Na poniższym przykładzie jedna zmienna jest reprezentowana przez zarówno kolor, kształt jak i wielkość punktu.

Zauważmy, że co byśmy nie wybrali, legenda automatycznie dopasowuje się do treści wykresu.
...
Zmieniając argumenty funkcji `aes()`, możemy wybierać sposób reprezentacji określonych zmiennych. Z czterech, które rozważaliśmy, która jest najlepsza?
...
Zobaczmy, jak skorzystać z innych geometrii. Za przykład posłuży nam wykres z napisami, któ®y można wykonać używając warstwy `geom_text()`. Szczegółowy opis tej geometrii, wraz z przykładami użycia, znajduje się na stronie http://docs.ggplot2.org/current/geom_text.html. W sekcji `Aesthetics` znaleźć można listę cech, które można modyfikować. Aby skorzystać z tej geometrii należy określić przynajmenij trzy cechy: `x`, `y` i `label`. 
...
Spróbujmy tego drugiego rozwiązania. Czytająd plik z pomocą dla geometrii `geom_text()` można zauważyć, że za pozycjonowanie odpowiadają cechy `hjust` (w poziomie) i `vjust` (w pionie). Zobaczmy jak wyglądać będzie użycie tych argumentów.
...
Stosowanie etykiet na wykresie ma wady i zalety. Do głównych zalet zaliczyć można:

1. Nazwy obiektów pozwalają zorientować się co reprezentuje  dany punkt. Jest to przydatne, jeżeli wykres wykorzystujemy do identyfikacji *ciekawych* obserwacji, np. najbardziej odstających, najbardziej charakterystycznych.
2. Jeżęli etykiety są zbyt i przestają być czytelne, to można je skracać do kilku pierwszych liter np. funkcją `substr()`.
3. Jeżeli wykres przedstawia tylko kilka obserwacji, to nazwy lepiej *wypełniają* wykres, nie pozostawiając dużych pustych przestrzeni, które źle wyglądają wizualnie.
...
Na poniższym przykładzie mapujemy zmienną `druzyna` na cechę `color` i to mapowanie dotyczy obu geometrii, a więc i punkty i napisy będą miały kolory zależne od zmiennej `druzyna`.
...
Pierwsze zetknięcie z pakietem `ggplot` wymaga często przestawienia się na nowy sposób myśłenia o wykresach. Ale gdy już się w ten sposób wejdzie, to okazuje sie, że poznawsze dwie geometrie, z kolejnych korzysta się już naprawdę prosto. 

Przykładowo, geometria `geom_line()` służy do rysowania linii. Aby ją wykorzystać na wykresie, wystarczy ją dodać do wykresu znakiem `+`. 
...
Z punktu widzenia programu R, to równie dobrze możemy te same dane (dwie zmienen liczbowe) przedstawić używając punktów jak i używając linii.
...
1. Chcemy przedstawić trend, pokazzać jak pewna wartość zmienia się w czasie. Standardowo na osi poziomej `x` przedstawia się czas a na osi pionowej `y` wartość.
...
Jest to o tyle ciekawy przykład, że wymagać będzie wskazania inej cechy niż `y`, tak się bowiem składa, że wstęgi są opisane przedziałem, czyli cechami `ymin` i `ymax`. Każdą z tych cech możemy połączyć ze zmienną lub przypisać do nich stałą wartość. 
...
Przykładowe odpowiedzi znajdują się na stronie https://rawgit.com/pbiecek/MOOC/master/0_dane/9_zadania_sezon2.html
