...
Aby odtworzyć przykłądy z tego odcinka potrzebny jest pakiet `SmarterPoland`. Informacja o tym jak go zainstawlować jest umieszczona w odcinku drugim *Jak zainstalować R, RStudio oraz dodatkowe pakiety?*.
...
To, w jaki sposób tworzą one wykres yjaśnimi na kolejnym slajdzie o tytule *Elementy wykresu punktowego*, w tym miejscu, przyjrzyjmy się wyłącznie wynikom, które obie instrukcje produkują.
...
Dla wybranych gatunków, na wykresie punktowym przedstawiamy zależność pomiędzy wagą a prędkością osobników tego gatunku. Lżejsze gatunki to głównie ptaki, które są też szybsze niż duże koty, więc przedstawiany wykres pokazuje, że im cięższy osobnik, tym niższa jego prędkość maksymalna.

Pomimo jednak tego, że zależność przedstawiona na obu wykresach jest ta sama, ponieważ przedstawiają one te same dane, to jednak wykresy te wyglądają różnie.

Przyjrzymy się wynikowym wykresom oraz postarajmy się znaleźć jak najwięcej różnic i zastanówmy się, który sposób prezentacji jest lepszy.
...
Wykres punktowy stosuje się najczęściej w sytuacjach

Dla pewnego zbioru obiektów mamy dwie zmienne ilościowe (np. dla państw to może być PKB i średnia długość życia, dla osób to może być wzrost i waga, dla samochodów to może być przegieg i cena).

1. Nie wiemy czy i jakiej zalezności się spodziewać (czy i jak długość życia jest związana z PKB).
2. Wiemy, że dwie zmienne są zależne i chcemy przedstawić tę zależność (chcemy pokazać jak cena auta zmienia się z jego przebiegiem).
3. Wiemy, że w jednej zmiennej występują nietypowe obserwacje (np. bardzo duże) i chcemy zobaczyć jakim wartościom stowarzyszonej zmiennej one odpowiadają (np. czy osoby o bardzo wysokiej wadze to osoby bardzo wysokie czy nie).

Na wykresie punktowym można również zaznaczyć grupy, pozwala to dodatkowo na porównanie grup pod względem przedstawianych zmiennych. Taki wykre spunktowy z grupami warto stosować gdy
...
2. Wiemy, że grupy się różnią i chcemy pokazać w jaki sposób (np aby pokzać że kobiety są średnio niższe i znacznie lżejsze niż mężczyźni),
...
Zauważmy, że zmienne `waga`, `predkosc` i `druzyna` to kolumny występujące w zbiorze danych `koty_ptaki`. Zaś `x`, `y` i `shape` to cechy wykresu punktowego. Funkcja `aes()` wskazuje, które cechy wykresu przedstawiją które zmienne.
...
1. Nazwy obiektów pozwalają zorientować się czemu odpowiada dany punkt. Jest to przydatne, jeżęli wykres wykorzytujemy do identyfikacji *ciekawych* obserwacji, np. najbardziej odstających, najbardziej charakterystycznych.
2. Nazwy mna skracać do kilku pierwszych liter, przez co nie zajmują dużo miejsca.
3. Jeżeli wykres przedstawia tylko kilka obserwacji, to nazwy lepiej *wypełniają* wykres, nie pozostawiając dużych pustych przestrzeni.

A główne wady to:

1. Jeżeli obiektów jest dużo, to nazwy na siebie nachodzą, przez co mogą stać się nieczytelne.
2. Jeżeli nazwy są długi to może nie być oczywiste, gdzie jest punkt który przedstawia współrzędne na wykresie i czy to jest środek napisu czy któ®yś róg napisu.
...
Z punktu widzenia danych, to równie dobrze możemy te same dane przedstawić używając punktów jak i używając linii.

Jest jednak duża różnica w sposobie w jaki będziemy takie wykresy postrzegać.

Puntky nie mają kierunku, więc chmara punktów na wykresie pozoli ocenić czy te punktu ukłądają się wzdłuż jakiejś krzywej (jest zależność pomiędzy zmiennymi), czy tworzą kilka grup (są skupiska), czy występują wartości odstające.

Linie na wykresie mają kierunek, patrząc na linie nasze oko ocenia ich względne długości oraz względne kierunki, szacunjąc kąty pomiędzy liniami. Nie jesteśmy w stanie odróżnić małych różnic w kątach, próg rozróżnialności to około 4 stopni ale jesteśmy w stanie rozróżnić duże zmiany.

Z tego powodu, wykresów liniowych najlepiej używać gdy

1. Chcemy przedstawić trend, jak pewna wartość zmienia się w czasie, standardowo na osi poziomej przedstawia się czas a na osi pionowej wartość.
2. Chcemy przedstawić tempo zmiany trendu, czy tempo wzrostu rośnie czy maleje (czy kąty są coraz większe czy nie)
3. Chcemy pokazać zmienność pomiędzy sąsiednimi pomiarami. Ponieważ są siednie pomiary połączone są linią, będzie widać czy ta linia gwałtownie czy spokojnie zmienia kierunki.
