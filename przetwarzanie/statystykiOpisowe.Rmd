...
Przypuśćmy, że chcemy pokazać wyłącznie średnią. Jedną liczbę. Zrobmy to za pomocą wykresu słupkowego.
...
Powyżej pokazaliśmy wynik funkcji `summary()`, prezentujących sześć charakterystycznych informacji o rozkłądzie (min, max, kwartyle i średnią).
Pięć z nich, za wyjątkiem średniej, przedstawia wykres pudełko-wąsy, który w R można wykonać używając funkcji `boxplot()`.

W poniższym przykładzie przedstawiamy te pięć liczb dla zmiennej `Przebieg.w.km`. Argument `horizontal=TRUE` powoduje, że wykres rysowany jest poziomo, argument `range = 0` powoduje, że nie są wyznaczane wartoścvi odstające (a nie mówiliśmy co to za wartości, więc ich nei wyznaczajmy, wrócimy do tego tematu później).
...
Z wykresu tego możemy łatwo odczytac, że 3/4 danych dotyczy aut o przebiegu poniżej 200 tys km. Jedynie 1/4 danych ma wyższe wartości i dla tych aut rozpiętość jest bardzo duża.
...
Pierwsze oznacza rzeczy stojace pionowo, drugie oznacza zapis. W sumie chodzi o przedstawienie danych z użyciem pionowych słupków.
