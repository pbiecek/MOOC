...
+ `r median(dane2012$Rok.produkcji, na.rm=TRUE)` rok

## Brakujące dane
Może się tak zdarzyć, że oferta sprzedaży nie zawiera wszystkich informacji o aucie. Np. brakuje inforamcji o przebiegu. 
...
Często chcielibyśmy policzyć średnią, medianę, minimum wartości które są znae.
...
### Odpowiedzi:
+ Dla zmiennej `Przebieg.w.km`  jest `r sum(is.na(dane2012$Przebieg.w.km))` 20 711 brakujących wartości. Ta zmienna jest najmniej kompletna.
+  `r median(dane2012$KM, na.rm=TRUE)` cm3
...
+ Zmienna `Liczba.drzwi` przyjmuje wartości `2/3` i `4/5`. Zachowuje się ona jako zmienna jakościowa. Jednak można uznać, że liczba drzwi to cecha ilościowa, ponieważ opisuje _ilość_ drzwi w samochodzie. Jak wyjaśnić tę dułalność?
...
Funkcja `prop.table()` dzieli wartości tabeli liczebności przez sumę liczbności, a więc zamienia liczebności na frakcje.
Przypiszmy wynik funkcji `table()` do jakiejś zmiennej, np. o naziwe `tabela` (nazwa może być dowolna, musimy jej jednak konsekwentnie używać w dalszych przykładach).
Następnie tak otrzymaną tabelę liczbności zamieniamy na frakcje.

W poniższym przykłądzie, instrukcję przypisania wyniku obejmujemy napisami, po to by jej wynik był wyświetlony. Bez nawiasów wynik funkcji `table()` nie zostałby wyświetlony.
...
Zazwyczaj chcemy mieć wyznaczone procenty a nie frakcje. I to nie byle jakie procenty, ale procenty które są ładnie sformatowane, np. wyświetlone z dokładnością do jednego miejsca dzisiętnego po przecinku.
...
Ostanim usprawnieniem tego procesu, które warto rozważyć to uporządkowanie wartości rosnąco lub malejąco.
