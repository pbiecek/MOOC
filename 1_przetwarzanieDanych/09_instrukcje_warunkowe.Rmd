...
Mnogość dostępnych algorytmów do analizy danych stawia nas często przed koniecznością wyboru. Czy użyć metody A czy metody B, każda jest lepsza w innej sytuacji. Najlepszym rozwiązaniem jest często warunkowe użycie jednej lub drugiej metody w zależności od tego jak wyglądają nasze dane.
...
Poniższy kod wypisze na ekranie napis `Orzeł` jeżeli wylosuje się liczba mniejsza od 0.5 oraz napis `Reszka` jeżeli wylosuje się liczba większa równa 0.5.
...
Jeżeli pracujemy na wektorach, wygodnym rozwiązaniem jest skorzystaniem z instrukcji wektorowej `ifelse()`.
