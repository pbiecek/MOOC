...
date: "sezon 1 / odcinek 9 <br><br><br><br>Naciśnij `A` by zamienić slajdy na ciągły tekst. <br> Naciśnij `T` by wyświetlić spis treści."
output:
  slidy_presentation:
    includes:
...
Mnogość dostępnych algorytmów do analizy danych stawia nas często przed koniecznością wyboru. Czy użyć metody A czy metody B, każda jest lepsza w innej sytuacji. Najlepszym rozwiązaniem jest często warunkowe użycie jednej lub drugiej metody w zależności od tego jak wyglądaną nasze dane.
...
Przygotowując program, często jesteśmy w sytuacji w której dalsza akcja zależy od pewnej wartości, dla nas nieznane lub potencjalnie zmieniającej się.

Przykładowo, jeżeli losujemy liczbę losową z przedziału 0-1 i w zależności co się wylosowało chcemy napisać `Orzeł` lub `Reszka`, możemy nie wiedzieć jaka wartość się wylosuje.

Używając instrukcji warunkowych możęmy opisać alternatywne scenariusze, które będą wykonane w zależności od określonego warunku.

Poniższy kod wypisze na ekranie napis `Orzeł` jeżeli wylosuje się liczba mniejsza od 0.5 oraz napis `Reszka` jeżeli wysoluje się liczba większa równa 0.5.
...
Nawet gdy wyrażenie jest jedną instrukcją, to dodawanie nawiasów klamrowych `{}` jest popularną praktyką,
powoduje, że kod jest czytelniejszy. 
...
Jeżęli pracujemy na wektorach, wygodnym rozwiązaniem jest skorzystaniem z instrukcji wektorowej `ifelse()`.

Jej pierwszym argumentem może być wektor. Przykłądowo instrukcja `koty_ptaki[,"druzyna"] == "Kot"` tworzy wektor wartości logicznych.
...
for (i in 1:nrow(koty_ptaki)) {
  # wewnątrz pętli wykonanie zalęży od warunku i instrukcji if
  if (koty_ptaki[i,"druzyna"] == "Kot") {
...
Innym częstym zastosowaniem instrukcji warunkowych jest zmiana wartości zmiennej na inną wartość w zależności od tego jaki wystapił warunek.
...
for (i in 1:nrow(koty_ptaki)) {
  if (koty_ptaki[i,"druzyna"] == "Kot") {
    liczbaKotów  <- liczbaKotów + 1
  } else {
    liczbaPtaków <- liczbaPtaków + 1
  }
}
...
```{r, eval=FALSE}
# W pętlach, gdy dla róznych wierszy należy wykonać inne operacje wygodnie jest korzystać 
# z instrukcji warukowych
for (i in 1:nrow(koty_ptaki)) {
  # wewnątrz pętli wykonanie zalęży od warunku i instrukcji if
  if (koty_ptaki[i,"druzyna"] == "Kot") {
    cat(koty_ptaki[i,"gatunek"], "to wielki i szybki kot.\n")
...
# W instrukcjach warunkowych nie tylko możemy wyświetlać rózne napisy,
#    ale również zmieniać wartości zmiennych.
# W poniższym przykładzie w zależności od tego jaki wiersz jest odczytywamy 
