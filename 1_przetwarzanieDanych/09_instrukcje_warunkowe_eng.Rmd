...
Je¿eli u¿yjemy instrukcji `if-else`, to gdy `warunek` oka¿e siê byæ fa³szywy wyœwietli siê wynik czêœci `else`.
If we use the command `if-else`, the part after the `else` will be executed when the condition turns out to be false. 
...
The example below presents a loop with a conditional command which changes the value of variable `liczbaKotów` in one case or changes the value of the variable `liczbaPtaków` in the other case. 
