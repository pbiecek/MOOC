...
Je?eli u?yjemy instrukcji `if-else`, to gdy `warunek` oka?e si? by? fa?szywy wy?wietli si? wynik cz??ci `else`.
...
The example below presents a loop with a conditional command which changes the value of variable `liczbaKot?w` in one case or changes the value of the variable `liczbaPtak?w` in the other case. 
