...
date: "sezon 1 / odcinek 3 <br><br><br><br>Naciśnij `A` by zamienić slajdy na ciągły tekst. <br> Naciśnij `T` by wyświetlić spis treści."
output:
  slidy_presentation:
    includes:
...
Pracując ze slajdami, możemy w każdej chwili nacisnąć przycisk `T`, rozwinie on spis treści, dzieki czemu łątwo nam będzie przejść do porządanego slajdu.

Pracując ze slajdami możemy w każdej chwili nacisnąć przycisk `A` aby zamienić slajdy na ciągły dokument.
...
Po zainstalowaniu przy pierwszym otwarciu program R będzie wyglądał w poniżej przedstawiony sposób.
...
Na slajdach wyniki instrukcji wprowadzonych do programy R przedstawimy poprzedziwszy je znakami `##`. Poniżej znajduje się zdjęcie pokazujące jak wygląda konsola.
...
Pierwsze zadania są proste, kolejne coraz trudniejsze. Warto zmierzyć się z nimi, jedynym sposobem by nauczyć się języka programowani jest ćwiczenie, ćwiczenie i jeszcez raz ćwiczenie.

Odpowiedzi do zadań umieszczone są na stronei kursu, można je w każdej chwili sprawdzić.

Zadanie do tego odcinak polega na uruchomieniu wszystkich przedstawionych instrukcji w programie RStudio i sprawdzeniu czy otrzymuje się te same wyniki.
