...
date: "sezon 1 / odcinek 13 <br><br> <a href='http://biecek.pl/'>Przemysław Biecek @ Uniwersytet Warszawski</a><br><br><br>Naciśnij `A` by zamienić slajdy na ciągły tekst. <br> Naciśnij `T` by wyświetlić spis treści."
output:
  slidy_presentation:
    includes:
...
Jednak równocześnie każdy rodzaj zmiennych w programie R może również przyjmować wartość nieustaloną `NA`. 
...
przez wektor wartości logicznych powoduje wyzerowanie określonych wartości (TRUE jest zamieniane na 1, FALSE na 0, a elementy obu wektorów są mnożone element po elemencie).
...
# a tutaj liczymy procentwartości
...
Ponieważ operatory logiczne `lub` oraz `i` są często wykorzystywane przećwiczymy je jeszcze na zbiorze danych o samochodach. 
...
```
Tabela liczebnosci (tabela krzyżowa) dla obu zmiennych
```{r, warning=FALSE}
