...
Najbliższe cztery odcinki przedstawią cztery podstawowe typy danych: ilościowe, jakościowe, napisy i wartości logiczne, nie wyczerpują one jeszcze listy wszystkich możliwych typów, można by wyróżnić jeszcze daty, dane cenzorowane oraz inne bardziej specjalistyczne typy, ale te cztery występują w 90% analiz więc na nich się skupimy.
...
Cechy ilościowe mogą być fizyczne (masa, długość, prędkość) lub nie (iloraz inteligencji, wielkość długu publicznego, poziom procent zgadzania się z poglądami jakiejś partii). 

Mogą przyjmować bardzo wiele różnych wartości (np. wzrost w milimetrach) lub tylko kilka (np. liczba rąk).

Ponieważ jednak są opisane liczbami, można na nich wykonywać kilka wspólnych operacji. Przyjrzyjmy się im.
...
Aby coś powiedzieć o każdej ze zmiennych wygodnie jest scharakteryzować je jedną lub kilkoma charakterystykami, takimi jak średnia czy mediana (mediana to wartość która pojawi się w środku gdyby te 13 liczb posortować).

Jak sprawdzić ile wynosi średnia lub mediana dla wagi? 
Aby policzyć średnią możemy wykorzystać funkcję `mean()` a aby wykorzystać medianę, funkcję `median()`.
...
Wczytując dane ze zbioru, którego sami nie tworzyliśmy, często chcemy zobaczyć skrajne wartości dla poszczególnych zmiennych. Skrajne czyli największe i najmniejsze.
...
Najbardziej typową statystyką opisową dla pary zmiennych ilościowych jest korelacja. Można ją wyznaczyć, używając funkcji `cor()`. Przykładowo, zobaczmy czy jest zależność pomiędzy rokiem produkcji a ceną.

```{r}
cor(auta2012$Rok.produkcji, auta2012$Cena.w.PLN)
```

Zależność jest dodatnia, a więc im rok produkcji wyższy (=auto jest młodsze) tym jest droższe. Korelacja przyjmuje wartości od -1 do 1. Wartość 0.34 to przeciętna korelacja, pewnie część z nas mogłaby się spodziewać wyższej. Ale tak się składa, że mamy tutaj wymieszane różne marki i modele. Funkcja `cor()` domyślnie liczy korelację Pearsona, ale może też liczyć inne korelacje, np. Spearmana. Co to dokładnie znaczy i jak z niej korzystać, to omówimy w sezonie 3.

Inną przydatną funkcją, jest możliwość sprawdzenia, ile wierszy, ile aut, ma określoną cechę większą lub mniejszą od jakiejś wartości. 
...
Dlatego gdy liczmy średnie lub inne statystyki z wektorów, które zawierają wartości nieokreślone to wynik też jest wartością nieokreśloną `NA`.
...
Zobaczmy jak wyglądają średnie ceny w grupach. Przykładowo w zależności od rodzaju paliwa. 

Do liczenia średniej w grupach wykorzystamy funkcję `tapply()`, jako pierwszy argument przyjmuje cechę ilościową, jako drugi informacje o grupach a jako trzeci nazwę zmiennej, która ma w każdej grupie być wykonana (i następnie dodatkowe argumenty). 
...
Na poniższym wykresie zauważyć można dwa ,,pagórki''. Jeden pobliżu 0 oraz drugi w pobliżu 180 tys. km. 
...
Okazuje się jednak, że ponieważ ofert sprzedaży aut jest bardzo wiele, lepiej nie rysować każdej oferty dużym kołem, ale pojedynczym punktem Można to osiągnąć dodając argument `pch="."`. Ponieważ i przebieg i cena auta zawierają pojedyncze bardzo duże wartości zamiast pokazywać je w standardowej skali, znacznie lepiej przedstawić je w skali logarytmicznej. To można osiągnąć dodając do wykresu argument `log="xy"`. Wartość `"xy"` oznacza, że obie osie należy zlogarytmować. 

Więcej informacji o wykresach rozrzutu i innych sposobach przedstawienia cechy ilościowej przedstawione jest w sezonie 3.
