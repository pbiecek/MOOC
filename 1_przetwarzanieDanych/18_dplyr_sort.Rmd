...
W codziennej pracy z danymi, i wygodniej, i szybciej jest wykorzystać dedykowaną funkcję `arrange()` z pakietu `dplyr`.
...
O ile wektor wartości można posortować funckją `sort()`, to do sortowania wierszy w ramce danych względe określonej kolumny wygodnie jest uzyć funkcję `arrange()`.
...
Sortując po kolumnie `druzyna` wiele wierszy ma te same wartości, i o ich końcowej kolejnosci decyduje zmienna `predkosc`.
...
Zmienna `Model` za wartości przyjmuje napisy, w tym przypadku jest sortowany w kolejności alfabetycznej.
...
Domyślnie wartości sortowane są rosnąco. Jeżeli chcemy zminić kolejnośc sortowania i sortować malejąco, to należy daną zmienną opakować funkcją `desc()`, tak jak na poniższym przykładzie.
