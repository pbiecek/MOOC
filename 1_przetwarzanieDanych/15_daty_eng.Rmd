...
output:
  slidy_presentation:
    ...
    css: ../style.css
    font_adjustment: 0
    footer: "Data CruncheRs / processing / dates"
...
```{r}
(time1 <- as.POSIXct("2015-02-13 12:56:26"))
(time2 <- as.POSIXct("14022015 12:56:26", format = "%d%m%Y %H:%M:%S"))
```
...
```{r}
(time1 <- as.POSIXlt("2015-02-13 12:56:26"))
(time2 <- as.POSIXlt("14022015 12:56:26", format = "%d%m%Y %H:%M:%S"))
```
...
```{r}
wday(time3, label = TRUE)
# week of a year, day of a month, month and year
week(time1)
day(time1)
month(time1)
year(time1)
...
The detailed description of the package `lubridate` can be found in the JSS's article at http://www.jstatsoft.org/v40/i03/paper
