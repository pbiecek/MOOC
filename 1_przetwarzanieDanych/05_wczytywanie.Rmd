...
- kolejne wiersze to dane w postaci kolumn liczb, które są rozdzielane znakiem `;` (średnik),
...
Operację przypisania wartości odczytanych przez funkcję do zmiennej można wykonać operatorem `<-` lub `=`.
...
- `file` - ścieżka do pliku z danymi, może być też adres URL. Jeżeli wartości argumentów podajemy w ich domyślnej kolejności, to możemy pominąć nazwy argumentów (dlatego w kolejnych przykładach nie bedziemy podawać nazwy tego argumentu).
- `header` - flaga określająca, czy pierwszy wiersz należy traktować jako nagłówek. W naszym przypadku `header=TRUE`.
- `sep` - znak rozdzielający kolumny. W naszym przypadku `sep=";"`, ale popularnymi separatorami są również znak tabulacji, przecinek lub spacja.
