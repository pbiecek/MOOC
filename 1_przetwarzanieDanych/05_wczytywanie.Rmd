...
date: "sezon 1 / odcinek 5<br><br> Wszelkie prawa zastrzeżone.<br> Wykorzystanie możliwe wyłącznie za zgodą autora.  <br><br><br><br>Naciśnij `A` by zamienić slajdy na ciągły tekst. <br> Naciśnij `T` by wyświetlić spis treści."
output:
  slidy_presentation:
    includes:
...
Dane mogą być przechowywane  w najróżniejszych formatach, takich jak baza danych, plik tekstowy, plik w formacie programu Excel (nowszym .xslx lub starszym .xls) lub plik w formacie innego programu do analiz danych (SAS, SPSS, itp). 
...
Ostatnim (po prawej stronie na poniższym schemacie) jest funkcja, która odczytuje dane, przetwarza i zamienia na postać zrozumiałą dla progrmau R (na poniższym schemacie ta funkcja to `read.table()`).
...
Operacje przypisania wartości odczytanych przez funkcję do zmiennej, można wykonać operatorem `<-` lub `=`.

W programie R, jeżeli wynik jakiejś funkcji nie jest przypisany do zmiennej to jest on wyświetlany na ekranie. Jeżeli jednak chcemy ten wynik wykorzystać w przyszłości, to należy go przypsać do zmiennej.
...
Do wczytywania danych z pliku tekstowego posłużyć może funkcja `read.table()`. Ponieważ pliki tekstowe mogą mieć bardzo różną postać funkcja ta ma wiele argumentów. Wybrane agrumenty przedstawia poniższa deklaracja (jest ich więcej, ale część pominiemy by nie zaciemniać obrazu)
...
Ponieważ dane mogą być zapisane z użyciem różnych formatów, argumenty tej funkcji określają format danych do wczytania. Znaczenie kolejnych argumentów to

- `file` - ścieżka do pliku z danymi może być też adres URL. Jeżeli wartości argumentów podajemy w ich domyślnej kolejności, to możemy pominąć nazwę arguumentów. Dlatego w kolejnych przykładach nie bedziemy podawać nazwy tego argumentu
- `header` - flaga określająca, czy pierwszy wiersz należy traktować jako nagłówek. W naszym przypadku `header=TRUE`.
- `sep` - znak rozdzielający kolumny. W naszym przypadku `sep=";"`, ale popularnym separatorem jest znak tabulacji, przecinek lub spacja
- `dec` - kropka dziesiętna, zazwyczaj jest to `.` lub `,`. W naszym przypadku `dec=","`
...
- `comment.char` - znak komentarza. Jeżeli w danych wystąpi ten znak to treść od tego znaku do końca lini będzie zignorowana
...
W programie R dostępnych jest kilka pakietów pozwalających na odczytywanie danych w formacie Excela. Najpopularnijesze pakiety to `gdata`, `xlsReadWrite`, `XLConnect`, `xlsx`. Różnią się one zewnętrznymi bibliotekami, które wykorzystują, przez co z niektórych może być łatwiej skorzystać pod Windowsem z innych pod Linuxem. 
...
Dane, które chcemy wczytać, w Excelu są dostępne pod adrese http://biecek.pl/MOOC/dane/koty_ptaki.xls i wyglądają następująco.
...
Zestawienie najpopularniejszych pakietów wczytujących danych z Excela prezentuje poniższa tabela.

pakiet | funkcja_odczyt | funkcja_zapis  | formaty | funkcjonalność | szybkość | jezyk
---           | ---    | ---    | --- | --- | --- | ---
...
*Uwaga!* Pakiet `SmarterPoland` nie jest dostępny w podstawowej dystrybucji programu R i trzeba go wcześniej zainstalować poleceniem `install.packages()`. Szczegółowa instrukcja, jak to zrobić jest przedstawiona w odcinku 2 *Jak zainstalować R, RStudio oraz dodatkowe pakiety?*.
...
```{r, eval=FALSE}
# wczytywanie danych z plików tekstowych i przypisaie tej wartości do zmiennej `koty_ptaki`
koty_ptaki <- read.table("http://biecek.pl/MOOC/dane/koty_ptaki.csv", 
                        sep=";", dec=",", header=TRUE)
...
# wyświetlenie zbiorów danych dostępnych w pakiecie `SmarterPoland`
data(package="SmarterPoland")

# wczytanie danych koty_ptaki z pakietu `SmarterPoland`
library(SmarterPoland)
koty_ptaki
