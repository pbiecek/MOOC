...
Ostatnim (po prawej stronie na poniższym schemacie) jest funkcja, która odczytuje dane, przetwarza i zamienia na postać zrozumiałą dla progrmau R.
...
Do wczytywania danych z pliku tekstowego służy funkcja `read.table()`, której wybrane agrumenty przedstawia poniższa deklaracja (jest ich więcej, ale część pominiemy by nie zaciemniać obrazu)
...
- `nrows` - maksymalna liczba wierszy do wczytania, domyślnie -1, czyli wczytaj wszystkie wiersze 
...
Funkcja `read.table()` ma wiele argumentów, ale nei musimy ich wszystkich określać. Wystarczy wskazać nazwę pliku oraz te argumenty, które powinny mieć inne wartości niż domyślne. 
...
# wczytanie danych z Eurostatu o wykorzystaniu różnych środków transportu w róąnych krajach
tsdtr210 <- getEurostatRCV("tsdtr210")
head(tsdtr210, 3)
summary(tsdtr210)
```
