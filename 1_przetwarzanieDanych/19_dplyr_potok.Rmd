...
Jeżeli chcielibyśmy otrzymać pełny przepływ w czytaniu od góry na dół potoku, to zamiast operatora przypisania `<-` możemy wykorzystać operatora `->` o identycznym znaczeniu a różniący się tylko kolejnością argumentów.
...
Operator `%>%` jest najczęściej wykorzystywany w pracy z funkcjami z pakietów `dplyr` i `ggvis`, ale można go oczywiście stosować do innych funkcji również. Koniec końców, jedyne co robi to przekazuje swój lewy argument jako pierwszy argument wyrażenia po prawej stronie.
