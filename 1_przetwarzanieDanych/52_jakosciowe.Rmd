...
**PCH: Nadal będę się upierał, że wczytywanie 70MB przez sieć na początku 
każdego rozdziału nie jest dobrym pomysłem. U mnie na przykład, przy przyzwoitym
łączu, na dwie próby obie skończyły się wczytaniem tylko części danych i komentarzem
(a nie błędem) mówiącym, że nie udało się wczytać całości pliku. Ramka istnieje, 
ale zawiera część danych. Myślę, że nie unikniesz w rozdziale o wczytywaniu danych
jakiegoś wprowadzenia na temat poruszania się po plikach lokalnych. A no i nie wspominam
już o tym ile trwa 'kompilacja' ze źródeł rmd.**
...
**PCH: Ja wczytuję dane z auta2012.rda i mam nieco inny wynik, tzn. mam dodatkowy, 'pusty', level (cały blok
poniżej):**
```
> levels(auta2012$Waluta)
[1] ""    "CHF" "CZK" "EEK" "EUR" "GBP" "HUF" "PLN" "USD"
> nrow(auta2012)
[1] 210134
> table(auta2012$Waluta)

          CHF    CZK    EEK    EUR    GBP    HUF    PLN    USD 
     0      5  19211      2   5480      5    630 184137    664 
```
**PCH: Czy to możliwe, że w csv i obiekcie w rda masz nieco inne dane?**  
...
+ Pewne zmienne, w zależności od kontekstu, mogą być traktowane i analizowane jak zmienne ilościowe lub zmienne jakościowe. Do nas należy wybór, która z tych opcji jest właściwsza. **PCH: I tu dalej - o tym,
że w naszym przykładzie 'Liczba.drzwi', mimo iż jest wyrażona liczbowo, jest 
cechą jakościową, np. nie ma w tym kontekście sensu liczenie średniej liczby drzwi 
dla wybranej marki samochodów.**
...
```
**PCH: Tu moim zdaniem warto umieścić - na przykładzie mnożenia przez 100 - dygresję 
na temat tego, jak działają operacje na wektorach w R**
...
**PCH: A tu kolejna dygresja, tym razem o indeksowaniu wektorów. A zatem pokazujemy
funkcję `order()`, pokazujemy jej wynik, czyli kolejność i pokazujemy jak działa
indeksowanie używając order + indeksowanie do zrobienia tego co sort**
...
**PCH: A u mnie znowu inaczej: 38.8851875 %**
...
Można je wykonać np. funkcją `table()`. **PCH: a nie np. funkcją `barplot()` ?**
...
**PCH: TODO:
* stringAsFactors i okolice - czasem (często?) tekst jest niepotrzebnie zamieniany
na factor, a czasem zmienne jakościowe występuję pod postacią tekstu. Warto znać
as.factor, as.character
**

[*] jak zbudować samemu tabelę kontyngencji (array i dim, dimnames),
[*] jak przekształcić data.frame (tzw. dane surowe) do tabeli kontyngencji np. xtable i odwrotnie,
[*] jak przekształcić data.frame do tabeli zliczeniowej tzn. as.data.frame(xtabs(~zm1+zm2)) i odwrotnie.
