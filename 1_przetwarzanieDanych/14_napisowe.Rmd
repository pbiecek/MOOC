...
przekształcić (np. data, którą można przekształcić na napis by wyłuskać rok i miesiąc).
...
Dane od których rozpoczniemy przykłady to `koty_ptaki` z pakietu `SmarterPoland`. Aby te dane wczytać, 
...
Użyjemy funkcji `as.character()` by przekształcić ją na napis. Otrzymane napisy zapiszemy w kolumnie `habitat_napis`.
...
Dlatego dalsze ćwiczenia ze zmiennymi ilościowymi przeprowadzimy na znaczeni większym zbiorze danych `auta2012`, z ponad 200 tysiącami wartości, 
...
W podobny sposób, można stworzyć dodatkowe kolumny określające, czy auto ma na wyposażeniu klimatyzację lub czy jest bezwypadkowe.
...
```
[PCH: tak ma być, ze nie znalazł informacji o bezwypadkowych?]
---
...
W pakiecie `SmarterPoland` jest również dostępny zbiór danych `imiona_warszawa` ze statystykami popularności imion nadawanych noworodkom w Warszawie w różnych okresach czasu.
...
+ Gdy pracujemy z napisami bardzo użyteczne są wyrażenia i metaznaki. Krótkie ale treściwe omówienie wyrażań regularnych, opracowane przez S. Jonesa znajsuje się w dokumencie ,,String manipulation in R'' http://www3.nd.edu/~sjones20/JonesUND/BioStats_files/RstringManipulation_2-6-13.pdf.
