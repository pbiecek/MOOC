...
Przykładowo, sprawdźmy które wiersze w kolumnie `habitat_napis` posiadają w nazwie znak `A`. Pierwszym argumentem funkcji `grep()` jest wzorzec, drugim wektor napisów. Możemy te argumenty podać w innej kolejności, ale wtedy zmuszenie jesteśmy do podawania ich nazwy.
...
Dwie kolumny `Wyposazenie.dodatkowe` oraz `Kolor` będą nas szcezgólnie interesowały, jeżeli chodzi o analizę napisów.
...
W sytuacjach, gdy chcemy przetwarzać napisy, musimy zmienne jakościowe przekształcić na napisy uzywając funkcji `as.character()`.
...
W analizie danych często stoimy przed potrzebą stworzenia nowych cech na bazie starych. Przykłądowo, jeżeli spojrzymy na wekotor kolorów samochodów, możemy uznać że interesującą cechą byloby określenie, czy lakier jest koloru matalicznego czy nie.

```{r, warning=FALSE}
sort(table(auta2012$Kolor_napis))
```

Można zrobić to na kilka sposobów. Najłatwiejszym jest użycie fucnkji `grepl()` o takich samych parametrach jak funkcja `grep()` z tym wyjątkiem, że wynikiem jest wektor wartości logicznych TRUE/FALSE określający czy dany wzorzec został odnaleziony czy nie w napisach.
...
W podobny sposób, można stworzyć dodatkowe kolumny określające, czy auto ma na wyposażeniu klimatyzację lub, że jest bezwypadkowe.

W drugim przykłądzie warto zwrócić uwagę na to, że argumenty są podane w innych niż domyślna, kolejności. Dlatego argument ze wzorcem musiał mieć dodatkowo wskazaną nazwę `pattern=`.

```{r, warning=FALSE}
auta2012$maKlimatyzacje <- grepl("klimatyzacja", auta2012$Wyposazenie.dodatkowe_napis)
table(auta2012$maKlimatyzacje)

auta2012$bezwypadkowy <- grepl(x=auta2012$Wyposazenie.dodatkowe_napis, pattern="bezwypadkowy")
table(auta2012$bezwypadkowy)
```
...
Funkcje w programie R, które operują na wektorach (większość) jeżeli mają dwa argumenty o różnej długości to krótszy argument zwielokratniają tylukrotnie by dorównał długością dłuższemu (tzw. recycling rule). Z tego powodu trzeci argument, pojedyncza kropka będzie zwielokrotniona i sklejona z każdym z dziesięciu elementów wektorów `litery` i `cyfry`.
...
W pierwszej linii skleimy oba wektory, używając za separator `': '`. Następnie zliczamy ile było wystąpień takich par i porządkujemy je w kolejności malejącej.
...
W pakiecie `SmaterPoland` jest również dostępny zbiór danych `imiona_warszawa` ze statystykami popularności imion nadawanych noworodkom w Warszawie w różnych okresach czasu.

Uzywając tego zbioru danych, możemy przetestowac regułę mówiącą, że imię dziewczynki kończy się na literkę 'a' a chłopca nie. Jakiś czas temu za jedyny wyjątek podawano męskie imię Bonawentura. Czy to się zmieniło?
...
# Liczby, cechy jakoścowe i napisy

Funkcja `as.character()` możę być przydatna jeszcz w jednej wytuacji, gdy chcemy zmienną jakościową przekształcić na zmienną liczbową. Nie powiniśmy tego zrobić bezpośrednio, ale poprzez funkcję `as.character()`.

Pokażmy ten problem na przykłądzie. Zamienimy zmienne na zmienne jakościowe funkcją `as.factor()` a następnie na liczby funkcją `as.numeric()`. Okazuje się jednak, że wynik nie jest zgodny z oczekiwaniami.
...
Zmienne jakościowe przez funckję `as.numeric()` zamieniane są na kolejne liczby całkowite. Jeżeli chcemy odzyskać liczby, które są treścią czynników, to musimy je wpierw zamienić na napisy, funkcją `as.character()`.
...
Przy przetwarzaniu większych korpiusów tekstu, pojawia się wiele interesujących tematów, takich jak analiza znaczenia analiza częstościowa, tagowanie itp.
...
+ Gdy pracujemy z dużymi korpusami, możemy przyśpieszyć obliczenia stosując pakiet `stringr`. Bardzo dobrze przygotowany materiał opisujący funkcej tego pakietu jest udostępniony przez Gastona Sancheza na stronie http://gastonsanchez.com/Handling_and_Processing_Strings_in_R.pdf.

+ Interesującą alternatywą wspierająca bardziej egzotyczne języki (bardziej niż angielski) jest pakiet `stringi`. Można o nim posłuchać na wystąpnieniu Marka Gągolewskiego z II Spotkania Entuzjastów R, materiał video dostępny na stronie http://smarterpoland.pl/SER/#SERII
