...
date: "sezon 1 / odcinek 8<br><br> Wszelkie prawa zastrzeżone.<br> Wykorzystanie możliwe wyłącznie za zgodą autora.  <br><br><br><br>Naciśnij `A` by zamienić slajdy na ciągły tekst. <br> Naciśnij `T` by wyświetlić spis treści."
output:
  slidy_presentation:
    includes:
...
Co jednak, gdybyśmy chcieli powtórzyć ją 10 000 razy? Z pewnością nikt z nas nie chciałby wpisywać przez kilka dni tych instrukcji, tym bardziej, że ten sam efekt można by osiągnąć prostą pętlą.
...
W poniższym przykładzie dla każdej kolumny z ramki danych `koty_ptaki` wyświetlamy listę różnych wartości, które występują w tej kolumnie. Na kolejnym składzie wyjaśnimy krok po kroku co się dzieje w tej funkcji.
...
Pierwsza instrukcja wykorzystuje funkcje `colnames()` do wyznaczenia nazw kolumn. Te nazwy zostaną przypisane do wektora `kolumny`, który następnie zostanie wyświetlony.
...
Pokażmy teraz przykład wykorzystania pętli dla każdego wiersza, a na początek dla wiersza o indeksie `i=1`.

Będziemy chcieli wyświetlić informację do jakiej wagi dochodzą przedstawiciele danego gatunku.
...
Sekwencja `1:nrow(koty_ptaki)` tworzy wektor liczb od 1 do 13 (tyle jest wierszy). Dla każdego wiersza uruchamiany instrukcję z poprzedniego slajdu, wypisującą wagę kolejnego gatunku.
...
Powtórzmy powyższą pętle przedstawiającą prędkość pierwszego organizmu dla każdego wiersza osobno, wyświetlając kropkami prędkość a na końcu dopisując nazwę gatunku.
...
# pętla w pętli pozwala na wykonanie pewnej isntrukcji dla każdej pary kolumn
for (i in colnames(koty_ptaki)) {
  for (j in colnames(koty_ptaki)) {
    cat("Kolumna '", i, "' i kolumna '", j, "' mają w sumie", nchar(i)+nchar(j), "znaków.\n")
  }
