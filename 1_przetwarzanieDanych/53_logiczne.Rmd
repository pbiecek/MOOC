...
Cechy logiczne w R przyjmowac mogą jedną z trzech wartości, `TRUE` (logiczna prawda), `FALSE` (logiczny fałsz) i `NA` (wartość nieustalona). 

Zamiast pełnych nazw `TRUE` i `FALSE` można również korzystać ze skrótów `T` i `F`.

Wyrażenia logiczne najczęściej składa się połączywszy operatorai logicznymi `|` (logiczne lub) i `&` (logiczne i).
...
Tablibczę logicznego dodawania i mnożenia podsumowuje poniższa instrukcja.
...
Zbudujmy zmienną `czy_to_kot`, wektor o wartościach logicznych `TRUE` i `FALSE`, następnie rpzedstawmy jej podsumowanie.
