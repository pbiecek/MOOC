...
Zauważmy, że w tym przykładzie znak `-` przed funkcją `c()` tworzącą wektor, powoduje zmianę znku wszystkich elementów wektora. 
...
Poniższy przykład wybiera wiersze dla czterech wskazanych gatunków oraz trzy wybrane kolumny. Jeżeli wiersze mają sensowne nazwy, to bardzo ułatwia to odwoływanie się do wierszy.
...
Możemy wykorzystać ten wynikik, aby posortować ramkę danych po określonej kolumnie. W przykładzie poniżej wykorzystujemy funkcję `order()` do wyznaczenia wektora `kolejnosc`. Któty następnie wykorzystamy do indeksowania ramki `koty_ptaki`.
