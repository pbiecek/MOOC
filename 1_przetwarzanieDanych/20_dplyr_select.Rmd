...
Pracując na danych o oferach sprzedaży aut można było odczuć jak niewygodna jest praca na danych które mają wiele kolumn, w sytuacji gdy potrzebujemy jedynie kilku z nich. W odcinku 7 pokazaliśmy jak wybierać kolumny spełniające określone kryterium, używając operatora `[`. 

W codziennej pracy aby wybrać kolumny z całego zbioru danych, i wygodniej, i szybciej jest wykorzystać dedykowaną funkcję `select()` z pakietu `dplyr`.
...
W przykładzie poniżej wyświetlimy nazwy kolumn w zbioze danych `koty_ptaki`. A następnie wybierzemy trzy z tych kolumn w kolejnosci
...
Jeżeli kolumn w zbiorze danych jest wiele to wymienienie ich nazw jedna po drugiej może być niewygodne. W tym przypadku wato wykrozystać operator `:`, który pozwala na wybó kolumn od wskazanej do wskazanej.
...
W funkcji `select()` można wykorzystac operator `-` aby wybrać z ramki danych wszystkie kolumny poza wskazanymi.
...
Zbiór danych `auta2012` jest dostępny po wczytaniu pakietu `SmarterPoland`. Poniżej przedstawiamy listę nazw kolumn z tego zbioru danych, więcej informacji o tym zbiorze danych znaleźć można na stronie https://rawgithub.com/pbiecek/MOOC/master/0_dane/0_dane.html
...
Na poniższym przykłądzie usuwamy 9 wskazanych kolumn.
...
Przykład poniżej wykorzystujemy funckję `martches()` aby wybrać wszystkie nazwy kolumn, które w nazwie mają wzorzec `aj`.
