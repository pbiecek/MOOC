...
date: "sezon 1 / odcinek 6<br><br> Przemysław Biecek @ Uniwersytet Warszawski<br><br><br>Naciśnij `A` by zamienić slajdy na ciągły tekst. <br> Naciśnij `T` by wyświetlić spis treści."
output:
  slidy_presentation:
    includes:
...
Na dysku w aktualnym katalogu roboczym powstanie plik `wazne_dane.csv`. 
...
W kolejnych liniach występuja po trzy wartości, pierwsza to nazwa wiersza a dwie kolejne to wartości pierwszej i drugiej kolumny.
...
*Uwaga 2* Posiadacze systemu OSX w wersji Yosemite (na dzień dzisiejszy, najnowszej) mogą mieć problem z włączniem pakietu `xlsx` jeżeli mają zainstalowaną złą (lub żadną) wersję programu Java. Należy zainstalować najnowszą wersję ze strony http://osxdaily.com/2014/10/21/get-java-os-x-yosemite/.
...
Odczytując dane funkcją `load()`, do programu R wczytywane są obiekty razem z nazwami (pisaliśmy o tym w odcinku http://pogromcydanych.icm.edu.pl/materials/1_przetwarzanie/05_wczytywanie.html). Dlatego możemy do jednego pliku zapisać kilka obiektów i po wczytaniu danych nie będzie kolizji nazw.
