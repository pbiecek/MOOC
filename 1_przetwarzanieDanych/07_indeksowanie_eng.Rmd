...
date: season 1 / episode 7<br><br><br><br>Press `A` to change slides into text <br> Press `T` to display table of contents.
output:
  slidy_presentation:
    css: ../style.css
    font_adjustment: 0
    footer: Data CruncheRs / data exploration / indexing
    highlight: default
...
# indexing rows using logical expression
najszybsze <- koty_ptaki$predkosc > 100
koty_ptaki[najszybsze, ]

# use negative values to drop elements
koty_ptaki[ -c(1, 3, 8:10), ]
```
...
- Select all the rows except "Sokoły" (falcons: "Sokół wedrowny" and "Sokół norweski", rows 11 and 12) from the data frame `koty_ptaki`.
