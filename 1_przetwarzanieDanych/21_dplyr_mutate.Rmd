...
Dodajmy dwie nowe zmienne, w jednej przeliczmy prędkość z kilometrów na godzinę na mile na godzinne, w drugiej przeliczymy prędkość na liczbę długości na sekundę (długość zwierząt jest w metrach). Nową zmienną można dodać funkcją `mutate()`. Pierwszym argumentem jest ramka danych a kolejne to deklaracje nowych zmiennych w postaci `nazwa.zmiennej = wzór.na.nową.zmiennę `. 
...
Zauważmy też, że część cen jest netto a część brutto. Przeliczmy wszystkie ceny na brutto dodając 23\% tam gdzie podana jest cena netto.
