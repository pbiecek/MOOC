...
Dodajmy dwie nowe zmienne, w jednej przeliczmy prędkość z kilometrów na godzinę na mile na godzine, w drugiej przeliczymy prędkość na liczbę długości na sekundę (długość zwierząt jest w metrach). Nową zmienną można dodać funkcją `mutate()`. Pierwszym argumentem jest ramka danych a kolejne to deklaracje nowych zmiennych w postaci `nazwa.zmiennej = sposóļ.wyznaczenia`. 
...
Funkcję `mutate()` można też wykorzystać do zmieny wartości w zmiennej, już obecnej w zbiorze danych. 

W tym celu wystarczy, że nową wartość przypiszemy do nazwy istniejącej kolumny.

W przykłądzie poniżęj przeliczamy długość na cale i zaokrąglamy wynik do całkowitych liczb.
...
Operacje tworzenia nowych zmiennych przećwiczymy też na znacznie większym i złożonym zbiorze danyc. Mianowicie na zbiorze danych o cenach ofertowych używanych samochodów.
...
Zauważmy też, że część cen jest netto a częśc brutto. Przeliczmy wszystkie ceny na brutto dodając 23\% tam gdzie podana jest cena netto.
...
Na poniższym przykładzie w jednym krkoku wykonujemy obie powyżej przedstawione transformacje.
...
Wykorzystajmy funkcję `mutate()` aby dodać kolumny określające czy dane auto ma klimatyzację, centralny zamek, autoalarm.



```{r, warning=FALSE}
autaZWyposazeniem <- auta2012 %>%
  mutate(Autoalarm = grepl())
```
