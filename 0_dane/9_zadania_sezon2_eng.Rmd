...
# Find a TV show with the steepest declining trend in ratings.
...
```{r, message=FALSE, eval=FALSE}
# Find a TV show with a substantitial declining trend in ratings.

# Find a TV show (other than _Breaking Bad_) with a substantial increasing
# trend in ratings.

# Solution: As in task 13, plus testing for extreme values.
# Caution: it can take quite some time!!!
```
...
# For every value of parent's height from the `galton` data calculate, using
# functions from package 'dplyr', by how much on average children are lower than
# parents
srednie %>%
  mutate(roznica = srednia - rodzic)

# Given linear model parameters _b0_ and _b1_ calculate estimated heights of
# chilren for every value of parent's height.
wspolczynniki = coef(lm(syn~rodzic, data=galton)
wspolczynniki

# Calculate differences between values predicted with the linear model and
# (conditional) means calculated for every parent group separately.
srednie %>%
