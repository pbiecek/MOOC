...
# Użycie funkcji sapply, cut i switch by zwięźle zastąpić dwa if'y
sapply(
  cut(koty_ptaki$waga, c(0,1,100,1000)),
  switch,
  "lekkie", "srednie", "ciezkie")
...
# Kategoryzacja
auta2012$Rok.produkcjiKat <- cut(auta2012$Rok.produkcji, c(1900, 1990.5, 1995.5, 2000.5, 2005.5, 2010.5, 2012.5), right = FALSE)
...
```{r, eval=FALSE}
# najtańsze RR
tmp <- filter(auta2012, Marka == 'Rolls-Royce') %>%
  arrange(Cena.w.PLN)
head(tmp,10)

# największe V
tmp <- filter(auta2012, Marka == 'Volkswagen') %>%
...
# pęd [w kg m/s]
koty_ptaki %>% 
  mutate(pęd = predkosc*waga*1000/3600) %>% 
  arrange(-pęd)
