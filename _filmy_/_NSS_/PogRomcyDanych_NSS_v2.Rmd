...
```{r 1_wykres_slupkowy, echo=FALSE, fig.align='center', fig.height=5, fig.width=7}
...
Co powinien zrobić każdy prawdziwy sondażysta przed rozpoczęciem badania? Powinien odpowiedzieć sobie na  trzy podstawowe pytania.   
...
# Zadania 6:
  ...
  1.  Odpowiedz na pytania związane z następującym badaniem: Firma EkstraSondaż przygotowała w 2015 r. raport „Młodzież kupjąca substancje psychoaktywne przez internet” dla Państwowego Zakładu Badań nad sieciami komputerowymi. Dane zostały zebrane od respondentów przez Internet. W badaniu wzięli udział uczniowie wybranych szkół w całej Polsce, którzy dostali adres internetowy oraz indywidualne hasło do strony z ankietą. Łącznie próba badawcza liczyła 1050 nastolatków - osób w wieku 13-16 lat. Spośród nich  560  zdeklarowało, że robi zakupy przez internet. W tej grupie 15 osób zadeklarowało, że zdarzyło im się kupić substancje psychotropowe (Pytanie brzmiało: „Czy zdarzyło Ci się kupić przez internet  substancje psychotropowe, w tym leki wpływające na funkcjonowanie mózgu?” ). Wiadomo również, że w badaniu wzięło udział 180 szesnastolatków, z czego 110 robi zakupy przez internet, a 10 deklaruje zakup substancji psychotropowych. 
    ...
    h. Czy znając wyniki badania zgodziłabyś/zgodziłbyś się z następującymi określeniami:    
      ...
      iii. “Nowe zjawisko w internecie. Gimnazjaliści kupują narkotyki w sieci" - TAK / NIE     
...
# sposób czytania arkuszy Google zapożyczony ze strony:
...
Powyższa tabela zawiera rozkład odpowiedzi na nasze pytanie wśród mieszkańców wsi oraz miast.  Widzimy, że w miastach mieszka 12 osób, na wsi 8. Głosować nie zamierza łącznie 10 osób, co oznacza, że absencja wyborcza w populacji wynosi 50%. W normalnych warunkach rozkład ten byłby dla nas zupełną tajemnicą. Odkrywamy go żeby zobaczyć jaki ma on wpływ na to co będzie się działo w dobieranych przez nas próbach.

Teraz potrzebujemy operatu losowania. W naszym przypadku wygląda on następująco:
 ...
 * (ABSENCJA, GŁOSOWANIE) lub (GŁOSOWANIE, ABSENCJA) => (50%) - jedna z dwóch osób NIE ZAMIERZA pójść na wybory.
...
barplot(c(10*9, 20*10, 10*9), las=1, names.arg = c("0%", "50%", "100%"), add=T, col="black", ylim=c(0, 230), ylab = "Liczba prób", xlab="Poziom absencji wyborczej w próbie", cex.names = 1.5 );
box( col = "grey90")
text(b.6, c(10*9, 20*10, 10*9)+10, labels = c(10*9, 20*10, 10*9), cex = 1.4);
...
#   geom_bar( fill=c("black","black", "black"), colour="black")+ # c("red2","deepskyblue", "yellowgreen")
#   ylab("Liczba prób")+
#   xlab("Poziom absencji wyborczej w próbie")+
#   ylim(0,225)+
#   geom_text(data=permutacje.rozklad.Tab ,aes(x=Var1, y=Freq+25, label=Freq),vjust=5, size=5, face="bold")+
#   theme(axis.text.x = element_text(face="bold", color = "black", size=15));
```
...
#   ylab("Liczba prób")+
#    geom_text(data=p6.2, aes(x = factor(p6.2), y = Freq+250000, label=Freq) ,vjust=0.1, size=3, face="bold")

par(mfrow=c(1,1), mar=c(4,7,1,0))
...
box( col = "grey90")
text( rozklad.p6, prawdopo[,3]+3*10^5, labels = prawdopo[,3],cex=1.3)
...
#   }
#   abline(v=7/20, col="green", lwd=4, lty=2)
...
# ![Symulacja ]( Losowanie_wielu_prob/losowanie_wielu_prob.png "SYMULACJA")
```
...
plot(6:20,
     ...
     xlab="Liczba respondentów próby",
     las=1,
     cex.lab=1.3,
     cex.axis=1.2)
rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col = 
...
points(6:20,
       ...
       pch=19,
       cex=2)
# ![Błędy oszacowania ]( Losowanie_wielu_prob/blad_oszacowania.png "Błędy oszacowania")
```
...
# abline(v=seq(6,20,1), col="grey100");
# box( col = "grey90");
# abline(h=7/20, col="green", lwd=4, lty=2);
...
for (i in 1:15) {
  lines( x=rep(i+5,2), lwd=6,
         y=rbind( proby.sumy_przedzial[i,3],
                  proby.sumy_przedzial[i,4]) )
  text(i+5, proby.sumy_przedzial[i,4]+15, paste(round(proby.sumy_przedzial[i,2],0), "%", sep=""), cex=1.3 )
...
# ![Przedziały ufności ]( Losowanie_wielu_prob/przedzial_ufnosci.png "Przedziały ufności")
```
