---
title: "Sondaże bez tajemnic"
author: "Jakub Rutkowski,<br/> Zbigniew Marczewski,<br/> Przemysław Kopa<br/>Na Straży Sondaży dla pogRomcy danych"
date: "><br><br>Naciśnij `A` by zamienić slajdy na ciągły tekst. <br> Naciśnij `T` by wyświetlić spis treści."
output:
  slidy_presentation:
    includes:
    ...
    css: ../../style.css
    font_adjustment: 0
    footer: "pogRomcy danych / modelowanie / sondaże"
...
Mamy nadzieję, że dzięki naszym materiałom przekonacie się, że prawdziwe sondaże to nie wróżenie z fusów, lecz rzetelna analiza danych uzyskanych przy pomocy metody reprezentacyjnej.
...
```{r 1_wykres_slupkowy, echo=FALSE, fig.align='center', fig.height=8, fig.width=10}
options(OutDec= ",", scipen=10); # scipen - zapis naukowy liczb
kolor<-"blue3";
kolor_osie<-"blue3";
kolor_label<-"blue3";
kolor_tekst<-"blue3";
...
# Zadania 6:         
  ...
  1.  Odpowiedz na pytania związane z następującym badaniem: Firma EkstraSondaż przygotowała w 2015 r. raport „Młodzież kpująca substancje psychoaktywne przez Internet” dla Państwowego Zakładu Badań nad sieciami komputerowymi. Dane zostały zebrane od respondentów przez Internet. W badaniu wzięli udział uczniowie wybranych szkół w całej Polsce, którzy dostali adres internetowy oraz indywidualne hasła do strony z ankietą. Łącznie próba badawcza liczyła 1050 nastolatków - osób w wieku 13-16 lat. Spośród nich  560  zdeklarowało, że robi zakupy przez Internet. W tej grupie 15 osób zadeklarowało, że zdarzyło im się kupić substancje psychotropowe (Pytanie brzmiało: „Czy zdarzyło Ci się kupić przez internet  substancje psychotropowe, w tym leki wpływające na funkcjonowanie mózgu?” ). Wiadomo również, że w badaniu wzięło udział 180 szesnastolatków, z czego 110 robi zakupy przez internet, a 10 deklaruje zakup substancji psychotropowych. 
    ...
    e. Jaki procent osób, które zadeklarowały w badaniu, że kupują środki psychotropowe to szesnastolatkowie?    
          ......................................................................................................................................................................................     
    f. Jaki procent nastolatków, które kupują coś przez internet, kupuje substancje psychotopowe?
          ......................................................................................................................................................................................          
    g. Czy znając wyniki badania uznałabyć/uznałbyć następujące naglówki prasowe za trafne:     
...
par(mar=c(5,4,1,0), col.axis = kolor_osie, col.lab=kolor_label)
b.6<-barplot(c(10*9, 20*10, 10*9), las=1, ylim=c(0, 230))
rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col = "grey90")
abline(h=seq(0,100,50), col="grey100");
abline(v=b.6, col="grey100");
barplot(c(10*9, 20*10, 10*9), las=1, names.arg = c("0%", "50%", "100%"), add=T, ylim=c(0, 230), ylab = "Liczba prób", xlab="Poziom absencji wyborczej w próbie", cex.names = 1.5, col=kolor   );
box( col = "grey90")
text(b.6, c(10*9, 20*10, 10*9)+10, labels = c(10*9, 20*10, 10*9), cex = 1.4, col=kolor_tekst);
```
...
barplot(p2, las=1, add=T, col=kolor, ylim=c(0, 230), ylab = "Liczba prób", xlab="Poziom absencji wyborczej w próbie", cex.names = 1.5 );
box( col = "grey90");
text(b.6, p2+500, labels = p2, cex = 1.4, col=kolor_tekst);
```

Jak widać wyniki symulacji nie odbiegają od tego, co przewidywaliśmy. w `r p2[[1]]  ` próbach absencja wyborcza wyniosła 0%, w `r p2[[2]]` wyniosła 50%, a w `r p2[[3]]` wyniosła 100%. Różnice są niewielkie. Widzimy więc teraz, że schemat losowania próby determinuje szansę uzyskania poprawnego oszacowania. W przypadku naszego badania wynoszą one 200/380, a więc około 53%.
...
Wróćmy do naszego nowego schematu losowania respondentów. Ogólnie rzecz biorąc generuje on aż 20\*19\*18\*17\*16\*15=`r 20*19*18*17*16*15` różnych prób. To jest znacznie, znacznie więcej niż w przypadku badania absencji wyborczej, gdzie dobieraliśmy 2 respondentów. Niestety nie mamy miejsca, żeby rozpisać wszystkie próby. Możemy natomiast opisać ich zbiorowość wykonując pewne obliczenia. Zacznijmy od tego, w ilu próbach pojawi się każdy obywatel. W przypadku schematu doboru dwóch respondentów każdy obywatel mógł utworzyć 19 par z innymi osobami z populacji i dodatkowo zająć pierwsze lub drugie miejsce w próbie. W konsekwencji występował w 19\*2=`r 19*2` próbach. A co by się stało, gdybyśmy dobierali trzyosobowe próby? W tym przypadku dla każdego obywatela można dobrać najpierw jedną osobę spośród 19, a później drugą spośród pozostałych 18. Obywatel mógłby przy tym zajmować pierwsze, drugie lub trzecie miejsce w próbie. W efekcie każdy występowałby w 19\*18\*3=`r 19*18*3` trzyosobowych próbach. Analogicznie w przypadku sześcioosobowej próby najpierw do każdego obywatela można dobrać jedną z 19 osób, potem jedną z 18 pozostałych osób, potem jeszcze jedną z pozostałych 17 osób itd., aż wreszcie ostatnią z pozostałych 15. Dodatkowo nasz obywatel może zając 1,2,3,4,5 lub 6 miejsce w próbie. Ostatecznie więc występuje w 19\*18\*17\*16\*15\*6=`r 19*18*17*16*15*6` sześcioosobowych próbach.      
...
Ostatecznie otrzymujemy więc 5\+4\+3\+2\+1=`r 5+4+3+2+1` różnych wersji początkowego układu preferencji (X,X,X,X,7,8) Pamiętajmy, że 7,8 to tylko jedna z 13\*12=`r 13*12` par dających dwie preferencje nX w próbie (inna może byc para 8,7). Dodatkowo na każdą z nich przypada 7\*6\*5\*4= `r 7*6*5*4` różnych układów respondentów tworzących preferencje (X,X,X,X). Ostatecznie więc prób, w których poparcie dla partii X wyniesie 66,7% (4/7) jest (7\*6\*5\*4)\*(13\*12)\*15=`r (7*6*5*4)*(13*12)*15`       
Na powyższy problem można też popatrzeć inaczej. Wystarczy zauważyć, że dla dwóch preferencji X mamy do przydzielenia łącznie 6 miejsc. Pierwsza z preferencji może zając jedno miejsce z 6, a druga jedno z 5 pozostałych. Łącznie możemy więc wyróżnić 6\*5=`r 6*5` różnych ich ustawień. Ale uwaga! Pamiętajmy , że kolejność respondentów musi pozostać niezmieniona. Jeżeli najpierw dobieramy obywatela 7, a potem 8 to znaczy, że akceptujemy próbę (7,..,..,..,..,8) a odrzucamy (8,..,..,..,..,7). Pozbywamy się więc wszystkich "duplikatów". W ten sposób otrzymujemy (6\*5)/2=`r (6*5)/2` różnych ustawień dla pary obywateli 7 i 8, przy zachowaniu ich kolejności. To jest dokładnie tyle ile otrzymaliśmy dla wcześniejszych obliczeń.      
A co z próbami, w których poparcie dla partii X wynosi 50% (3/6). Na podobnej zasadzie jak wcześniej możemy policzyć, że dla prób, w których preferencje występują w kolejności (X, X, X, nX, nX, nX) jest (7\*6\*5)\*(13\*12\*11)=`r (7*6*5)*(13*12*11)` . Następnie liczmy na ile sposobów możemy "wymieszać" każdą taką próbę, nie burząc jednocześnie porządku wśród respondentów o preferencjach X oraz nX oddzielnie. Pierwsza z preferencji nX może zająć jedno z 6  miejsc, druga jedno z pozostałych 5, trzecia jedno z pozostałych 4 - łącznie 6\*5\*4=`r 6*5*4`. Musimy już tylko pozbyć się "duplikatów". Trzy preferencje nX można potencjalnie ustawić na 3\*2\*1=`r 3*2` sposobów. Nas interesuje tylko jedna z nich, bo nie chcemy zmieniać kolejności preferencji. W związku z tym respondentów nX można "ustawić" nie na 6\*5\*4=`r 6*5*4` tylko na (6\*5\*4)/6=`r (6*5*4)/6` sposobów. Szacowana poparcie dla partii X na poziomie 50% (3/6) wystąpi więc w (7\*6\*5)\*(13\*12\*11)\*(6\*5\*4)/6=`r (7*6*5)*(13*12*11)*(6*5*4)/6` próbach.      
...
```{r,echo=FALSE, fig.align='center', fig.height=7, fig.width=13}
par(mfrow=c(1,1), mar=c(4,7,1,0), col.axis = kolor_osie, col.lab=kolor_label)
rozklad.p6<-barplot( prawdopo[,3], las=1, ylim=c(0,1.2*10^7), xlab="Poparcie w próbie", cex.lab=1.5, cex.axis=1.2, axes=FALSE)
rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col = "grey90")
abline(h=seq(0,10*10^6,2*10^6), col="grey100");
abline(v=seq(0,8,1), col="grey100");
barplot( prawdopo[,3], las=1 , add=T, col=kolor)
box( col = "grey90")
text( rozklad.p6, prawdopo[,3]+5*10^5, labels = prawdopo[,3],cex=1.3, col=kolor_tekst)
title(ylab="Liczba prób",mgp=c(5,1,0))
```
...
par(mar = c(4.2,4.2,1,1), mfrow=c(2,2), col.axis = kolor_osie, col.lab=kolor_label);
for (i in c(1,4,7,10)) {
  plot(c(-100,100), 
       c(-100,100), 
       ylim=c(0,50), 
  ...
  box( col = "grey90");
  points( proby[[i]], pch=19, col=kolor)
  for( j in 1:dim(proby[[i]])[1] ){
    lines( rep(proby[[i]][j,1],2)*100, c(0, proby[[i]][j,2])*100, lwd=5 , col=kolor)  
  }
  abline(v=7/20*100, col="green", lwd=4, lty=2)
  text(50, 40, paste("Liczba\nrespondentów:", i+5, sep="\n"), adj=0, cex=1.6, col=kolor_tekst)
}
```
...
par(mar = c(4.2,4.2,1,1), col.axis = kolor_osie, col.lab=kolor_label);
plot(6:20,
     sqrt(unlist(lapply( proby, FUN = function(x) { sum( (x[,1]-0.35)^2*x[,2] )})) ),
     ylim=c(0,0.2),
     ylab="Odchylenie standardowe",
     xlab="Liczba respondentów w próbie",
     las=1,
     cex.lab=1.3,
     cex.axis=1.2)
rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col = 
...
par( mfrow=c(1,1), col.axis = kolor_osie, col.lab=kolor_label);
plot(proby.sumy_przedzial[,1], proby.sumy_przedzial[,3], ylim=c(0,100), 
...
abline(v=seq(6,20,1), col="grey100");
box( col = "grey90");
abline(h=7/20*100, col="green", lwd=4, lty=2);
points( proby.sumy_przedzial[,1], proby.sumy_przedzial[,3], pch="-", cex = 6, col=kolor);
points( proby.sumy_przedzial[,1], proby.sumy_przedzial[,4], pch="-", cex = 6, col=kolor);
for (i in 1:15) {
  lines( x=rep(i+5,2), lwd=6,
         y=rbind( proby.sumy_przedzial[i,3],
                  proby.sumy_przedzial[i,4]), col=kolor )
  text(i+5, proby.sumy_przedzial[i,4]+15, paste(round(proby.sumy_przedzial[i,2],0), "%", sep=""), cex=1.3, col=kolor_tekst )
  #text(i+5, 100, paste(round(proby.sumy_przedzial[i,2],1), "%", sep=""), cex=1.3 )
  text(i+5, proby.sumy_przedzial[i,4]+5, paste(round(proby.sumy_przedzial[i,4],0), "%", sep=""), cex=1, col="grey20")
  text(i+5, proby.sumy_przedzial[i,3]-5, paste(round(proby.sumy_przedzial[i,3],0), "%", sep=""), cex=1, col="grey20")
}
```
...
# Zadania 8:      
  ...
  3. Dla wylosowanej wcześniej próby (_Zadania 7._) oblicz błąd standardowy oszacowania poziomu poparcia dla partii X:     
      ...
      ......................................................................................................................................................................................          
      ......................................................................................................................................................................................               
  4. Czy szacowany poziom poparcia dla partii X w twojej próbie jest nie mniejszy niż 20% i nie większe niż 40%?    
    + Tak
    + Nie    
...
### Tabela 9.2. Wyniki z prób sześcioosobowych przy niepełnej realizacji (mieszkańcy miast są niedostępni)      
...
### Tabela 9.4. Poparcie dla partii X w próbach sześcioosobowych przy niepełnej realizacji (niedostępni mieszkańcy miast)     
...
par(mfrow=c(1,1), mar=c(5,5,0,0), col.axis = kolor_osie, col.lab=kolor_label);
plot( realizacja$Group.1, realizacja$procent, las=1, ylab="Odsetek prób", xlab="Oszacowanie w próbie", cex.lab=1.5, cex.axis=1.2)
rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col = 
       "grey90")
...
#points( realizacja$Group.1, realizacja$procent, pch=15, cex=2)
box( col = "grey90")
for( i in 1:dim(realizacja)[1]) {
  lines( rep(realizacja$Group.1[i],2), c(0,realizacja$procent[i]), lwd=11, col=kolor)
}
...
# Zadania 9:      
  ...
  1. Z wylosowanej wcześniej próby 15 osób (_Zadania 8._) wyłącz wszyskie osoby zamieszkałe w mieście. Podaj liczby porządkowe osób, kótre zostały w próbie.     
      ......................................................................................................................................................................................               
  ...
  4. Czy wynik z twojej próby mieści się w przedziale (17%, 83%)?    
    + Tak
    + Nie          
  5. Czy wiesz juz jak powstają sondaże?    
    + Tak
    + Nie        
