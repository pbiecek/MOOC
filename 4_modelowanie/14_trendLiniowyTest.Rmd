...
date: "sezon 2 / odcinek 14<br><br> Wszelkie prawa zastrzeżone.<br> Wykorzystanie możliwe wyłącznie za zgodą autora.  <br><br><br><br>Naciśnij `A` by zamienić slajdy na ciągły tekst. <br> Naciśnij `T` by wyświetlić spis treści."
output:
  slidy_presentation:
    includes:
...
W poprzednim odcinku poznaliśmy metodę wyznaczania współczynników trendu liniowego. 

Dzięki temu, możemy np. ocenić czy jakiś obserwowany parametr średnio rośnie czy maleje.

Czasem jednak ocena współczynnika $a_1$, opisująca tempo wzrostu jest bardzo bliska wartości $0$. Możemy wtedy zadać dodatkowe pytanie, czy obserwowany wzrost nie jest wynikiem przypadkowych fluktuacji?

Jak odpowiedzieć na to pytanie? Kiedy obserwowany trend jest przypadkowy a kiedy nie?

I tutaj ponownie pojawia się statystyka. Używając takich narzędzi jak *test dla współczynnika trendu* możemy ocenić na ile wielkość współczynnika wzrostu jest nieprzypadkowa.
...
Będziemy testować hipotezę (=przypuszcznie), czy współczynnik wzrostu jest istotnie różny od zera, czyli od braku trendu. 
...
To liczenie wartości trendu dla losowo pomieszanych danych realizowane przez poniższy kod.
...
Możemy teraz wartość `modulBreakingBad` porównać z wektorem wartości referencyjnych - `trendy`.
...
Tylko 0.00001? Czyli, gdy zaburzaliśmy losowo oceny to nie udało się uzyskać tak dużego wzrostu jak w przypadku serialu *Breaking Bad*. Ta jedna wartość, przez którą obserwowana średnia różni się od zera, to wynik trendu obserwowanego bez permutacji, który dodaliśmy do wektora `trendy`.

To porównanie, pokazujące jak dużą wartością `modulBreakingBad` na tle przypadkowego `trendy` jest przekonującym argumentem za ty, że obserwowany wzrost ocen dla *Breaking Bad* nie jest przypadkowy.
...
Hipoteza, którą chcemy sprawdzić, to czy ten trned istotnie większy od zera? 
...
Zweryfikujemy hipotezę o ostotności trendu w ten sam sposób co dla serialu *Breaking Bad*.

Przeprowadźmy te same obliczenia, a wyniki porównamy w ten sam sposób co porzednio.
...
Już graficzna ocena sytuacji, pokazuje, że mamy do czynienia z inną sytuacją.Tym razem dosyć często się zdarza, żeby losowe permutacje wektora ocen owocowały większym współczynnikiem wzrostu niż ten, który obserwujemy dla serialu Przyjaciele.

Jak opisać te różnice jedną liczbą?

```{r, cache=TRUE}
# jak często obserwuje się wartość większą lub równą
# p-wartość
mean(trendy >= modulFriends)
```

Dla serialu *Friends* nawet gdy losujemy wartości ocen w ponad 17 procentach przypadków obserwujemy trend większy niż `0.00043`. Czy to oznacza, że rosnący trend jest czysto przypadkowy? 

Pewności nie ma, ale raczej jesteśmy skłonni uznać, że obserwowany trend nie jest istotnie różny od zera. 

Za próg przyjmuje się często 0.05 (głównie z powodów historycznych, ale już tak się utarło). Co znaczy tyle, że uznaje się trend za nieprzypadkowy jeżeli jest mniejszy o co najwyżej 5\% wartości, które można by przypadkowo zaobserwować (jedna na 20).

Przyjmując za próg 0.05, p-wartość którą obserwujemy dla trendu dla sezonu Przyjaciele jest wyższa, więc przyjmiemy hipotezę, że ten trend nie jest isotnie różny od zera.

# Podsumowanie testowania

Wyniki, które otrzymaliśmy dla serialu *Breaking Bad* są inne niż dla serialu *Friends*.

W przypadku serialu *Breaking Bad* próbowaliśmy 99 999 razy losowo pomieszać dane by na pomieszanych zaobserować wzrost większy, niż w przypadku oryginalnej kolejności. To się nie udało, co przekonało nas, że obserwowany trend dla *Breaking Bad* jest *istotnie większy od przypadkowego (=0)*.

W przypadku serialy *Friends* próbowaliśmy tego samego. Z tą różnicą, że udawało się dosyć często w wyniku losowych permutacji uzyskać wartości większe, niż dla oryginalnej kolejności. 

Statystykę, opisującą jaka część losowo wygenerowanych trendów jest większa, niż zaobserowowany trend, nazywamy p-wartością. Przyjeliśmy też, że jeżeli p-wartość będzie mała, np. mniejsza niż 0.05 (arbitralny wybór, ale się przyjeło) to uznamy, że trend jest istotny. 
