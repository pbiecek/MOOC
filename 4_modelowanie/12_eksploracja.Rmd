...
Zanim zacznie się budować modele, zanim rozpocznie się analizę danych, wcześniej warto przyjrzeć sie tym danym bliżej.
...
Na potrzeby tego odcinka z Internetu pobraliśmy dane, oczyściliśmy je i udostępniliśmy w zbiorze danych `serialeIMDB` w pakiecie `PogromcyDanych`, który towarzyszy temu kursowi. 
...
Dla kilku pierwszych wierszy przyjrzyjmy się tym kolumnom po kolei.
...
Kolumna `serialeIMDB$serial` zawiera nazwy seriali, często powtarzające się.

Użyteczną funkcją do eksploracji zmiennych jakościowych jest tabela częstości. Przedstawia ona jak często określona wartość występuje w wektorze.
...
Praca na tak dużym zbiorze danych, o wszytkich serialach jest trudna, ponieważ nie jesteśmy w stanie ogarnąć wzrokiem wszystkich 20 tysięcy wierszy.

Dlatego na potrzeby dalszych przykładów wybierzmy z tego zbioru danych jeden serial, któremu przyjrzymy się bliżej. Poniżej będzie to *Breaking Bad*, ale warto poeksperymentować również z innymi serialami.
...
Najłatwiej posortować odcinki po ocenie i wyświetlić taki posortowany zbiór danych. Na początku będą te o skranie niskich ocenach, na końcu te o skranie wysokich ocenach.
...
Każdy z nich ma określoną średnią ocenę, ale czasem chcielibyśmy myśleć o wszystkich odcinakch nich jak o pewnej całości. Czy cały serial jest dobry czy nie?

Jak podsumować oceny z 62 odcinków? 

Dwa najpoopularniejsze podejscia to średnia oraz mediana (czyli element środkowy). Można je prosto wyznaczyć funkcjami `mean()` i `median()`.
...
Mediana to wartość, która po postorowaniu jest w środku (lub dla parzystej liczby wartości, jest to średnia z dwóćh wartości po środku).

Więcej inforamcji niż sama średnia, niesie tak zwanych pięć liczb Tukeya. Można je wyznaczyć (razem ze średnią arytmetyczną) funkcją `summary()`. Te pięć liczb przedstawia: 
...
Pomiędzy sąsiednimi poziomymi liniami mieści się tyle samo wartości. Jeżeli jednej z tych odcinków jest dłuższy niż pozostałę, to oznacza że wartości w nim są rzadsze niż w innych.
...
Co możemy odczytać z takiej reprezentacji danych? Naprawdę wiele rzeczy. Spróbujmy zmierzyć się z tym zagadnieniem przez chwilę samdzielnie. Na poniższym wykresie znajdźmy trzy ciekawe obserwacje.
...
Co ciekawego udało się znaleźć? Dla mnei najciekawsze obserwacje to:
...
2. Najsłabsze wyniki notuje sezon 3. W nim to znajduje się odcinek najgorzej oceniany. Też dla niego mediana jest najniżej.
...
Dzielimy zakres wartości na pewną liczbę (poniżej na `breaks = 10`) przedziałów o równej długości. Następnie w każdym przedziale sprawdzamy ile wartości do niego wpada. Na wykresie wysokością słupka oznaczamy liczbę tych obserwacji.
