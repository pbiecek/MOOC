...
```{r}
table(TCGA_BRCA$czy.nowy.guz,
      TCGA_BRCA$czy.zyje)
```

```{r}
przerzuty_przerzycia <- table(TCGA_BRCA$czy.nowy.guz,
      TCGA_BRCA$czy.zyje)
```

```{r}
prop.table(przerzuty_przerzycia, 1)
```

```{r}
frakcje <- prop.table(przerzuty_przerzycia, 1)
frakcje[,1]/frakcje[,2]
```
...
```{r}
fisher.test(przerzuty_przerzycia)

fisher.test(przerzuty_przerzycia)$p.value
```
