...
date: "sezon 2 / odcinek 17<br><br> Wszelkie prawa zastrzeżone.<br> Wykorzystanie możliwe wyłącznie za zgodą autora.  <br><br><br><br>Naciśnij `A` by zamienić slajdy na ciągły tekst. <br> Naciśnij `T` by wyświetlić spis treści."
output:
  slidy_presentation:
    includes:
...
W tym odcinku podejmiemy się analizy statystycznej tego zbioru danych. Ale analizowanie wszystkich ofert w w jednym worku nie ma sensu. 
Dlatego dalsze analizy przeprowadzimy wyłącznie na podzbiorze danych dotyczącym marki Volkswagen.

Dlaczego tej? Jest ona najpopularniejsza w zebranych ogłoszeniach, jest dla niej najwięcej obserwacji. Co nie bez znaczenia wyniki wychodzą ciekawe.
...
Wiedząc, że te oferty były zbierane w styczniu 2012 roku łatwo nam będzie wyznaczyć wiek auta (2012 - rok produkcji).

Zacznijmy więc od wyznaczenia zmiennej wiek auta (funkcja `mutate()` w poniższym przykładzie), a następnie usunięcia aut ponad 20 letnich (funkcja `filter()`, bardzo starych aut jest niewiele, są nietypowe i będą nam przeszkadzać w modelowaniu).
...
Mając współczynniki, możemy ocenić że średnia cena nowo używanego auta według modelu liniowego to 67430 oraz co roku auto traci średnio -4220 pln na wartości. Te dwie liczby są użyteczne, ale jeszcze więcej mozna odczytać z wykresu.
...
Zobaczmy najpierw, jak te dane wyglądać będą, jeżeli ceny przedstawimy w skali logarytmicznej. Dodając funkcje `scale_y_continuous()` zmieniamy osie wykresu. Do punktów dorysujemy linię regresji.
...
Na rysunku, po zlogarytmowaniu danych, ta zależność wygląda na idealnie liniową.

# Logarytmiczna / multiplikatywna w R

Zbudowac taki model multiplikatywny z użyciem funkcji `lm()` należy wpierw musimy policzyć logarytm ceny.
...
Zobaczmy jak wygląda przegieg i wiek aut.
...
Przyglądając się dokłądniej, okazuje się, że są wśród naszych ofert auta o absurdalnym przebiegu ponad 15 milionów kilometrów. To ewidentny błąd. 

Usuńmy wszystkie wiersze , których przebieg przekracza czterysta tysięcy kilometrów.
...
Po oczyszczeniu danych narsujmy zależność pomiędzy przebiegiem a wiekiem raz jeszcze.
...
Trend wygładzony i liniowy odstają od siebie znacząco. Co to oznacza? Być możę zalężność w danych wcale nie jest liniowa.
