...
Wciąż nie wiemy, czy `0.01834001` to duży wzrost czy mały. Musimy go z czymś porównać.
...
* [I] Wymieszamy losowo wartości ocen. Dzięki temu otrzymamy sekwencje ocen o której wiemy, że nie ma w niej żadnego systematycznego trendu, a ewentualna wielkość współczynika trendu wynika z losowych fluktuacji,
* [II] Policzymy jak duży jest trend dla tych nowo wylosowanych grup,
* [III] Powtórzymy kroki [I] i [II] wielokrotnie aby zobaczyć jak duże wartości może przyjmować trend w sytuacji gdy jest on wynikiem losowych fluktuacji.

To liczenie wartości trendu dla losowo pomieszanych danych realizowane przez poniższy kod.

```{r, eval=FALSE}
trendy <- replicate(100000, {
  # wymieszaj losowo wartości
  tylkoBreakingBad$wymieszane <- sample(tylkoBreakingBad$ocena)

  # moduł współczynnika wzrostu dla losowo wymieszanych danych
...
Możemy teraz wartość `modulWzrostu` porównać z wektorem wartości `trendy`. 
Wektor wielkości wzrostów jakich spodziewać się możemy gdy próby różnią się wyłącznie przypadkową zmiennością. Gdyby `modulWzrostu` był znacząco większy niż wartości z wektora `trendy`, to byłby to silny argument za tym, że trend rosnący w ocenie odcinków *Breaking Bad*  nie jest przypadkowy.
...
abline(v=modulWzrostu, col="red", lwd=5)
```

Jak widzimy, obserowana wartość wzrostu ocen dla serialu *Breaking Bad* jest znacznie większa niż wzrosty, które byłoby widać, gdyby nie było żadnego trendu.

# Czy wzrost ocen dla Breaking Bad to przypadek? Test permutacyjny

Wykres pomaga w zrozumieniu tego rozkładu, ale aby podjąć decyzje lepiej mieć jedną liczbę. Policzmy jaka część trendów dla losowych fluktuacji jest większa niż obserwowany trend dla *Breaking Bad*.

```{r, eval=FALSE}
mean(trendy > modulWzrostu)
```

Zero? Gdy zaburzaliśmy losowo oceny to nie udało się uzyskać tak dużego wzrostu, co jest przekonującym argumentem za ty, że obserwowany wzrost ocen dla *Breaking Bad* nie jest przypadkowy.

# A co z Przyjaciółmi?

A jak wygląda trend dla serialu *Friends*? 

Również jest dodatni, ale czy istotnie większy od zera? 

Jest mniejszy niż dla *Breaking Bad*, ale czy wciąż istotny?

```{r}
tylkoFriends <- serialeIMDB %>%
...
Przeprowadźmy teraz te same obliczenia co dla *Breaking Bad* ale dla serialu *Friends*.
...
Dla serialu *Friends* nawet gdy losujemy wartości ocen w kilkunastu procentach przypadków obserwujemy trend większy niż `0.00043`. Czy to oznacza, że rosnący trend jest czysto przypadkowy? 

Pewności nie ma, ale raczej jesteśmy skłonni uznać, że obserwowany trend nie jest istotnie różny od zera. 
Za próg przyjmuje się często 0.05 (głównie z powodów historycznych, ale już tak się utarło). Co znaczy tyle, że uznaje się trend za nieprzypadkowy jeżeli jest mniejszy o co najwyżej 5\% wartości, któ®e można by przypadkowo zaobserwować.
 
```{r}
mean(trendy > modulWzrostu)
# wykres z wartościami
hist(trendy, 50, col="grey", main="", las=1, border="white", xlim=c(0, 0.02))
abline(v=modulWzrostu, col="red", lwd=5)
```
...
* Znajdź serial (inny niż _Breaking Bad_) o istotnie dodatnim trendem. 
