...
wieku. Metody te były z początku wykorzystywane w nawigacji oraz astronomii. Dzisiaj najczęściej modele liniowe określa się terminem regresja.
...
przedstawił wyniki badań nad dziedziczeniem wzrostu. 

Galton zauważył, że synowie
...
ojców jest bliższy średniej w populacji. To zjawisko nazwał ,,dążeniem do przeciętności'' lub ,,tendencją do przeciętności'' (w języku angielskim ten współczynnik
nazywa się *regression toward the mean*). 

Galton zaproponował równanie
...
Zbiór danych nazywa się `galton` i jest dostępny w pakiecie `PogromcyDanych`. Zobaczmy kilka pierwszych obserwacji z tego zbioru.
...
W kolumnie `rodzic` przedstawiony jest średni ważony wzrost rodziców (średnia wzrostu
ojca i 1,08 wzrostu matki), a w kolumnie `syn` wzrost dorosłego syna. Wzrosty podane są w centymetrach (oryginalnie Galton przedstawiał wzrost w calach, dla nas jednak wygodniejsze są centymetry).
...
```{r, warning=FALSE, width=7, height=7}
...
```{r, warning=FALSE, width=7, height=8}
srednie <- galton %>%
  group_by(rodzic) %>%
...
Aby opisać zależność posługując się średnimi w każdej grupie, należałoby podać średnią dla każdego możliwego wzrostu rodziców.
...
Zapis `1 + parent` oznacza, że średni wzrost dziecka chcemy opisać za pomocą wyrażenia $1 * b_0 + parent * b_1$, w którym należy wyznaczyć $b_0$ i $b_1$.
...
1. Model, w którym średni wzrost dziecka jest równy średniemu ważonemu wzrostowi rodzica,
...
4. Model, w średni wzrost dziecka jest różny dla każdego wzrostu rodzica (niezależnie w każdej grupie rodziców liczymy średni wzrost dzieci).
...
```{r, warning=FALSE, width=7, height=7, echo=FALSE}
ggplot(galton, aes(x=rodzic, y=syn)) + 
  geom_point(position = "jitter")+
  # model 4, dla każdej grupy wzrostu rodziców wyznaczamy średni wzrost dzieci
  geom_line(data=srednie, aes(x=rodzic, y=srednia), size=2, color = "blue") + 
